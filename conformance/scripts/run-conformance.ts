...
async function main() {
  ...
  // Load test specification if provided (before plan determination)
  ...
  // Derive plan from spec file if --plan is not provided
  let planArg = args.plan as string | undefined;
  if (!planArg && testSpec) {
    // First, check for planKey in spec file (preferred, added in new spec format)
    const specWithPlanKey = testSpec as TestSpec & { planKey?: string };
    if (specWithPlanKey.planKey && TEST_PLANS[specWithPlanKey.planKey]) {
      planArg = specWithPlanKey.planKey;
      console.log(`Using plan key from spec file: ${planArg}`);
    } else {
      // Fallback: find plan by planName (for backward compatibility)
      for (const [key, plan] of Object.entries(TEST_PLANS)) {
        if (plan.name === testSpec.planName) {
          planArg = key;
          console.log(`Detected plan from spec file planName: ${planArg}`);
          break;
        }
      }
    }
  ...
  // Validate that plan is specified
  if (!planArg) {
    console.error('Error: --plan is required unless --spec is provided with planKey');
...
Options:
  -p, --plan <name>        Test plan to run. Required unless --spec is provided.
                           Plan can be auto-detected from spec file's planKey.
  ...
  -s, --spec <path>        Path to test specification JSON file
                           ...
                           If planKey is present in spec, --plan can be omitted
...
Environment Variables:
  ...
  ADMIN_API_SECRET         Required. Secret for authenticating with the Admin API
...
Examples:
  ...
  CONFORMANCE_TOKEN=xxx ADMIN_API_SECRET=yyy npx tsx conformance/scripts/run-conformance.ts --plan basic-op

  # Run with spec file (plan auto-detected from planKey in spec)
  CONFORMANCE_TOKEN=xxx ADMIN_API_SECRET=yyy npx tsx conformance/scripts/run-conformance.ts \\
    --spec ./conformance/scripts/specs/test-spec-basic-op.json
  ...
  CONFORMANCE_TOKEN=xxx ADMIN_API_SECRET=yyy npx tsx conformance/scripts/run-conformance.ts --spec ./test-spec.json

  # Run all tests with visible browser (for debugging)
  CONFORMANCE_TOKEN=xxx ADMIN_API_SECRET=yyy npx tsx conformance/scripts/run-conformance.ts --plan all --show-browser

  # Run against local environment
  CONFORMANCE_TOKEN=xxx ADMIN_API_SECRET=yyy npx tsx conformance/scripts/run-conformance.ts --plan basic-op --environment local
`);
}
