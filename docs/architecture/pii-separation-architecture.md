...
const idTokenClaims = {
  ...
  authrim_roles,
  authrim_user_type,
  authrim_org_id, // ✅ Non-PII
  authrim_org_name, // ⚠️ Quasi-PII (company name) - not included by default
  authrim_orgs, // ⚠️ Quasi-PII (contains org names) - not included by default
};
```
...
|                       | organizations               | Org name (※quasi-PII, needs review) |
...
| -------------- | ---------------------------------------------- | ----------------- |
| **USER_CACHE** | ✅ Full profile (email, name, phone, etc.)    | 🔴 Required       |
| CONSENT_CACHE  | ❌ scope, granted_at only                     | ✅ Not required   |
| STATE_STORE    | ❌ state → client_id                          | ✅ Not required   |
| NONCE_STORE    | ❌ nonce → client_id                          | ✅ Not required   |
| CLIENTS_CACHE  | ❌ Client configuration                       | ✅ Not required   |
| REBAC_CACHE    | ❌ Roles, permissions                         | ✅ Not required   |
...
- Cache tier is "fastest, cheapest, volatile" — can be rebuilt if lost
- Core tier is "durable, unique, consistent" — foundation of authentication
- PII tier is "regionally bound, recoverable" — addresses legal requirements
...
export const authorizeHandler: AuthHandler = async (c, ctx) => {
  ...
  // ctx.pii.getUserProfile(userId);  // ❌ Compile error: Property 'pii' does not exist
};
...
├─────────────────────────────────────────────────┤
│ Option A: Event-driven (adopted)                │
│   PIIRepository.update() →                      │
│     → CacheRepository.invalidate(userId)        │
│                                                 │
│ Option C: Version (combined)                    │
│   Add user_version column to Core DB            │
│   Embed version in Cache                        │
...
├─────────────────────────────────────────────────┤
│ 1. Circuit Breaker                              │
│    PII DB failure detection → auto degraded mode│
│    • N consecutive failures → Open state        │
│    • After M seconds → Half-Open → retry        │
│    • On success → return to Closed state        │
│                                                 │
│ 2. Graceful Degradation                         │
│    /userinfo → Return Core only if PII fetch    │
│    fails                                        │
│    { sub: "uuid", email: null, name: null }    │
│                                                 │
│ 3. Fallback Region (optional, OFF by default)  │
│    EU DB down → Read from US replica            │
│    ※ GDPR exception clause applies,            │
│      per-tenant opt-in                          │
│                                                 │
...
Fallback Region (EU DB down → US replica) is **OFF by default** for the following reasons:
...
// Using AuthHandler type → Accessing ctx.pii causes compile error
export const authorizeHandler: AuthHandler = async (c, ctx) => {
  // ✅ OK: Only Core and Cache accessible
...
// Using PIIHandler type → ctx.pii is accessible
...
├─────────────────────────────────────────────────┤
│ Level 1: Row-level (default)                    │
│   WHERE tenant_id = ?                           │
│   → All tenants share the same DB               │
│   → Most cost-efficient                         │
│                                                 │
...
│   → Can take individual backups                 │
│                                                 │
│ Level 3: Database-level (highest isolation)     │
│   tenant_abc → dedicated Postgres instance     │
│   → Complete physical separation                │
│   → "This tenant is EU-only" is easy            │
└─────────────────────────────────────────────────┘
```
...
│   • Keep user_id (for audit log references)    │
...
│                                                 │
│ Audit Log:                                      │
│   • Keep Core side (legal requirement)         │
│   • Anonymize PII side                         │
└─────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────┤
│ Rotation Schedule:                              │
│   • Master Key (KEK): Once per year            │
│   • Tenant DEK: Once per year or on-demand     │
│   • Index Key (Blind Index): No rotation       │
│                                                 │
│ Rotation Process:                               │
│   1. Generate new DEK                          │
│   2. Set old DEK to "decrypt only" mode        │
│   3. Re-encrypt in background                  │
│   4. Delete old DEK after completion           │
│                                                 │
│ Key Versioning:                                 │
│   encrypted_value = version:iv:ciphertext      │
│   Example: v2:abc123...:encrypted_email...     │
└─────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────┤
│ Metrics by Layer:                               │
│   • Core Layer: /authorize, /token p50/p99     │
│   • PII Layer: /userinfo, /admin p50/p99       │
...
- No JOINs available → Parallel queries with `Promise.all` (no issues in practice)
- No cross-DB transactions → State management via `pii_status`
...
```
New creation: pending → active (success) / failed (failure)
M2M: none (no PII)
Deletion: active → deleted (Tombstone created)
```
...
- Phase 8 (Policy Integration) prioritized Token Embedding and Check API (✅ Complete)
