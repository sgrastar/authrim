...
## 3.1 Internal JTI Generation in DO (V3 Implementation)

### Generation/ShardIndex Management on DO Side

The RefreshTokenRotator DO persists generation/shardIndex on the first `createFamily()` call.
This enables generating JTIs in the correct format during `rotate()`.

```typescript
// DO internal state
class RefreshTokenRotator {
  private generation: number | null = null; // Generation number
  private shardIndex: number | null = null; // Shard index

  // Restore from Durable Storage on initialization
  ...
  // Save generation/shardIndex on V3 request
  async createFamily(request: CreateFamilyRequestV3) {
    ...
    // ... rest of family creation processing
  }
}
```
...
sequenceDiagram
    ...
    DO->>DO: Persist generation=2, shardIndex=16
    DO-->>Admin: { version: 1, newJti: "v2_16_rt_abc" }

    Note over Client: 2. First refresh
    Client->>DO: POST /rotate<br/>incomingJti=v2_16_rt_abc
    DO->>DO: Match OK: v2_16_rt_abc === v2_16_rt_abc
    DO->>DO: newJti = v2_16_rt_newUUID (using generation/shard)
    DO-->>Client: { newJti: "v2_16_rt_newUUID" }

    Note over Client: 3. Second refresh
    Client->>DO: POST /rotate<br/>incomingJti=v2_16_rt_newUUID
    DO->>DO: Match OK: v2_16_rt_newUUID === v2_16_rt_newUUID ✅
```

### generateJti() Implementation

```typescript
private generateJti(): string {
  const randomPart = `rt_${crypto.randomUUID()}`;

  // Generate full JTI format when generation/shardIndex are set
...
```
✅ V3 Implementation Points:
├─ DO persists generation/shardIndex
├─ JTI generated by rotate() is in full format (v{gen}_{shard}_{random})
├─ op-token uses JTI returned from DO as-is (no wrapping needed)
└─ JTI comparison is done between full-format values

⚠️ Seed Creation Notes:
├─ Must pass generation/shardIndex to createFamily()
├─ Token registration API in admin.ts extracts generation/shardIndex from JTI
└─ Once DO saves generation/shardIndex, it won't change thereafter
```
...
- 2025-12-05: V1.1.0 - Added Section 3.1 (Internal JTI Generation in DO / V3 Implementation Details)
- 2025-12-04: V1.0.0 - Initial version (Generation-based Sharding Specification)
