...
**ユースケース例:**
1. **マイクロサービスアーキテクチャ**
   - シナリオ: 50個のAPIをKongで管理、Enraiで認証
   - 動作: Kong JWTプラグインでEnrai発行トークン検証、認証成功後API呼び出し

2. **レガシーシステム統合**
   - シナリオ: 既存APIはOAuth未対応、Enrai+Apigeeで認証レイヤー追加
   - 動作: Apigeeが認証、バックエンドAPIにはヘッダーでユーザー情報渡す

3. **マルチクラウド環境**
   - シナリオ: AWS、Azure、GCPに分散したAPI群
   - 動作: 各クラウドのAPI Gateway+Enrai JWT検証で統一認証
...
**ユースケース例:**
1. **Auth0からの移行**
   - シナリオ: 料金高騰により10万ユーザーをEnraiに移行
   - 動作: Auth0 Management APIでユーザーエクスポート→CSV変換→Enraiバッチインポート→検証
...
3. **ハイブリッド運用**
   ...
   - 動作: 新規ユーザーはEnrai、既存ユーザーは旧IdP→徐々にEnraiへ移行
...
**ユースケース例:**
1. **リスク最小化移行**
   - 週1: 内部テストユーザー10名→Enraiへ
   - 週2: 一般ユーザー10%（1万人）→Enraiへ
   - 週3: 25%→週4: 50%→週5: 100%
   - 各段階で監視、問題あればロールバック
...
| ストレージI/O | R2読み取り: エクスポートデータ、D1読み取り: Enraiデータ |
...
1. **移行後検証**
   - シナリオ: 10万ユーザー移行完了後
   - 動作: 旧IdPとEnraiのユーザーデータ比較→差分レポート生成→「98,523件一致、1,477件差分あり」
