#!/bin/bash
#
# TEST 2: Refresh Token Storm (Light プリセット)
#
# 使い方:
#   chmod +x run-test2-light.sh
#   ./run-test2-light.sh
#
...
RESPONSE=$(curl -s -X POST "${BASE_URL}/api/admin/clients" \
  ...
  -d '{"client_name":"Refresh Storm Test","redirect_uris":["https://localhost:3000/callback"],"grant_types":["authorization_code","refresh_token"],"scope":"openid profile email","is_trusted":true,"skip_consent":true}')
...
# Step 3: k6 テスト実行
# ============================================
echo "🚀 Step 3: TEST 2 - Refresh Token Storm を実行..."
echo "📊 プリセット: light (約5分間)"
echo ""
