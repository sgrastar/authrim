...
/**
 * Passkey ユーザーシード生成スクリプト
 *
 * Passkey登録済みユーザーを事前作成し、負荷テスト用データを生成する。
 * 各ユーザーに ECDSA P-256 鍵ペアを生成し、WebAuthn登録フローを実行する。
 *
 * 環境変数:
 *   BASE_URL             対象の Authrim Worker URL (default: https://conformance.authrim.com)
 *   ADMIN_API_SECRET     Admin API シークレット (required)
 *   PASSKEY_USER_COUNT   生成するユーザー数 (default: 100)
 *   CONCURRENCY          並列数 (default: 10)
 *   OUTPUT_DIR           出力ディレクトリ (default: ../seeds)
 *   USER_ID_PREFIX       ユーザー ID プレフィックス (default: pk-user)
 *
 * 使い方:
...
/**
 * COSE形式でpublic keyをエンコード（ES256 / P-256）
 * WebAuthn attestation で使用される形式
 */
...
/**
 * WebAuthn attestationObject を構築（none attestation）
 */
...
/**
 * WebAuthn authenticatorData を構築（登録時）
 */
...
/**
 * WebAuthn clientDataJSON を構築（登録時）
 */
