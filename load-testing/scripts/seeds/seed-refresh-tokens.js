 ...
 *
 * V3: Sharding support
 * - JTI format: v{generation}_{shardIndex}_{randomPart}
 * - Shard distribution via generation management
 *
 * V2: rtv (Refresh Token Version) claim support
 * High-speed generation via local signing + Admin API parallel registration
 *
 ...
 *
 * Note: In V2/V3, only one token family is maintained per user ID,
 *       so each token is assigned a different user ID ({USER_ID_PREFIX}-{index}).
 *
 * Usage:
...
if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error('❌ CLIENT_ID and CLIENT_SECRET are required. Set environment variables.');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`   CLIENT_ID      : ${maskValue(CLIENT_ID)}`); // lgtm[js/clear-text-logging]
  console.log(`   USER_ID_PREFIX : ${USER_ID_PREFIX}`); // lgtm[js/clear-text-logging]
  console.log(`   COUNT          : ${COUNT}`);
  console.log(`   CONCURRENCY    : ${CONCURRENCY}`);
  console.log(`   OUTPUT_DIR     : ${OUTPUT_DIR}`);
  console.log(`   User IDs       : ${USER_ID_PREFIX}-0 ~ ${USER_ID_PREFIX}-${COUNT - 1}`); // lgtm[js/clear-text-logging]
  console.log('');
