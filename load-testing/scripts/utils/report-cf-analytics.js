 ...
 *
 * Fetches comprehensive Workers Analytics using Cloudflare GraphQL API.
 * Integrates metrics from Workers, Durable Objects, D1, and KV.
 *
 * Required environment variables:
 *   CF_API_TOKEN: Cloudflare API Token (requires Analytics read permission)
 *   CF_ACCOUNT_ID: Cloudflare Account ID (required)
 *   WORKER_SCRIPTS: List of Worker names (comma-separated, optional)
 *
 * Usage:
 *   node scripts/fetch-cf-analytics.js --start "2025-11-30T10:20:00Z" --end "2025-11-30T10:35:00Z"
 *   node scripts/fetch-cf-analytics.js --minutes 10  # Last 10 minutes
 *   node scripts/fetch-cf-analytics.js --minutes 10 --json  # Output in JSON format
 */

const ACCOUNT_ID = process.env.CF_ACCOUNT_ID;
const API_TOKEN = process.env.CF_API_TOKEN;

// Worker names specified via environment variable (comma-separated) or defaults
const WORKER_SCRIPTS = process.env.WORKER_SCRIPTS
  ? process.env.WORKER_SCRIPTS.split(',')
  : [
      // Default: Authrim standard configuration
      // Can be overridden via WORKER_SCRIPTS environment variable
      'op-token',
      'op-auth',
      'op-userinfo',
      'op-management',
...
// Durable Objects Analytics query
...
// KV Analytics query (to be implemented after confirming available fields)
async function fetchKVAnalytics() {
  // KV Analytics has limited available fields in the current schema
  // Planned for future expansion
  return null;
}

// Aggregate and structure all metrics
...
function formatReport(metrics) {
  ...
  output += '═══════════════════════════════════════════════════════════════════════════════\n';
  output += `  Period: ${metrics.period.start} ~ ${metrics.period.end}\n`;
  output += '───────────────────────────────────────────────────────────────────────────────\n\n';

  // Worker Metrics
  output += '📗 Worker Metrics\n';
  output += '─────────────────────────────────────────────────────────────────────────────\n';
  output += `  Total Requests:        ${metrics.worker.total_requests.toLocaleString()}\n`;
  output += `  Subrequests:           ${metrics.worker.total_subrequests.toLocaleString()}\n`;
  ...
  // Durable Objects Metrics
  output += '📙 Durable Objects Metrics\n';
  output += '─────────────────────────────────────────────────────────────────────────────\n';
  output += `  Total Requests:        ${metrics.durable_objects.requests_total.toLocaleString()}\n`;
  output += `  Errors:                ${metrics.durable_objects.errors.toLocaleString()}\n`;
