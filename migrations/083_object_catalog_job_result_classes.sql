-- Expand object_catalog object_class values for generic job results and DR bundles.
-- SQLite/D1 cannot alter a CHECK constraint in place, so rebuild the table.
...
INSERT INTO object_catalog (
  id, public_artifact_id, tenant_id, object_class, created_at, updated_at, deleted_at
)
SELECT id, public_artifact_id, tenant_id, object_class, created_at, updated_at, deleted_at
...
INSERT INTO object_catalog_objects (
  id, catalog_id, representation, object_kind, object_index, bucket_binding,
  object_key, key_version, checksum_sha256, total_bytes, created_at, deleted_at
)
SELECT
  id, catalog_id, representation, object_kind, object_index, bucket_binding,
  object_key, key_version, checksum_sha256, total_bytes, created_at, deleted_at
