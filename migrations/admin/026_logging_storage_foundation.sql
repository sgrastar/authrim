...
CREATE UNIQUE INDEX IF NOT EXISTS idx_admin_destinations_scope_name_active
  ON admin_destinations(scope_type, scope_id, name)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_storage_destination_assignments_scope
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_logging_destination_overrides_effective
  ON logging_destination_overrides(COALESCE(tenant_id, 'platform'), log_type, plane)
  WHERE enabled = 1;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_logging_quota_policies_scope
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_admin_logging_sensitive_detail_policy_scope
  ON admin_logging_sensitive_detail_policies(log_type, plane)
  WHERE deleted_at IS NULL;
