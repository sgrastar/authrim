...
describe('runtime feature flags helper', () => {
	...
	it('renders token exchange only at platform scope', () => {
		expect(shouldRenderRuntimeFeatureFlag(RUNTIME_TOKEN_EXCHANGE_FEATURE_KEY, 'platform')).toBe(
			true
		);
		expect(shouldRenderRuntimeFeatureFlag(RUNTIME_TOKEN_EXCHANGE_FEATURE_KEY, 'tenant')).toBe(
			false
		);
