export const FORM_LOCALIZATION_LANGUAGES = [
	'en',
	'ja',
	'zh_CN',
	'zh_TW',
...
export type FormLocalizationLanguage = (typeof FORM_LOCALIZATION_LANGUAGES)[number];
...
const FORM_TEXT_LOCALIZATION_ALIASES: Record<string, string[]> = {
...
for (const [key, localizations] of Object.entries(FORM_TEXT_LOCALIZATIONS)) {
	...
	for (const alias of FORM_TEXT_LOCALIZATION_ALIASES[key] ?? []) {
		if (!canonicalByDefaultText.has(alias)) canonicalByDefaultText.set(alias, key);
	}
}

export function isDefaultFormText(value: string | null | undefined): boolean {
	return typeof value === 'string' && canonicalByDefaultText.has(value);
}

export function localizeDefaultFormText(
	value: string | null | undefined,
	language: FormLocalizationLanguage
): string {
	if (!value) return '';
	const canonical = canonicalByDefaultText.get(value) ?? value;
	return FORM_TEXT_LOCALIZATIONS[canonical]?.[language] ?? value;
}

export function mergeLocalizedDefaultFormText(
	current: string | null | undefined,
	source: string | null | undefined,
	language: FormLocalizationLanguage
): string {
	if (!source) return current ?? '';
	const localized = localizeDefaultFormText(source, language);
	const canonical = canonicalByDefaultText.get(source) ?? source;
	const defaults = new Set([
		canonical,
		...Object.values(FORM_TEXT_LOCALIZATIONS[canonical] ?? {}),
		...(FORM_TEXT_LOCALIZATION_ALIASES[canonical] ?? [])
	]);
