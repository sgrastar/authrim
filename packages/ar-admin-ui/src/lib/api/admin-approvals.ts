...
export const adminApprovalsAPI = {
	async list(params?: {
		...
		const response = await adminFetch(`${API_BASE_URL}/api/admin/approvals${query ? `?${query}` : ''}`)
	...
	async getStepGuide(requestId: string, approvalId: string) {
		...
		return parseResponse<ApprovalStepGuideResult>(
			response,
			'Failed to load approval step guide'
		)
	},
