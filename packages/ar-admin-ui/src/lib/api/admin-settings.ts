...
/**
 * Check if a setting is managed on a dedicated page
 * Settings with visibility: 'page' are displayed with toggles on their respective management pages
 * (e.g., ABAC toggle on Attributes page, ReBAC toggle on ReBAC page)
 */
export function isPageManagedSetting(settingMeta: SettingMetaItem): boolean {
	return settingMeta.visibility === 'page';
}
