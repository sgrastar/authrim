<script lang="ts">
	...
	/**
	 * JSON 文字列をサニタイズ（JSON インジェクション対策 - Medium 11）
	 * - JSON.parse でバリデーション
	 * - オブジェクトの深さ制限
	 * - プロパティ数制限
	 */
	function _sanitizeJSON(jsonStr: string): string {
		const MAX_DEPTH = 5;
		...
		try {
			...
			const checkDepth = (obj: unknown, depth = 0): boolean => {
				...
				return keys.every((key) => checkDepth((obj as Record<string, unknown>)[key], depth + 1));
			};

			if (!checkDepth(parsed)) {
				console.warn('[Security] JSON complexity limit exceeded');
				return '{}';
			}

			return JSON.stringify(parsed, null, 2);
		} catch {
			return jsonStr; // パースエラーは handleSave でキャッチ
		}
	}
	...
	function handleSave() {
		...
		try {
			// JSON インジェクション対策: 深さと複雑さをチェック（Medium 11）
			...
			if (!checkDepth(config)) {
				configError = 'JSON too complex (max depth: 5, max properties: 50)';
				return;
			}
	...
	function getNodeDescription(type: GraphNodeType): string {
		const descriptions: Partial<Record<GraphNodeType, string>> = {
			...
			check_auth_level:
				'Verify if user meets the required authentication level (basic, MFA, etc.).',
	...
	function updateDecisionBranch(index: number, field: keyof DecisionBranch, value: unknown) {
		...
		const sanitizedValue =
			field === 'label' && typeof value === 'string' ? sanitizeInput(value, 100) : value;

		decisionBranches = decisionBranches.map((b, i) =>
			i === index ? { ...b, [field]: sanitizedValue } : b
		);
		updateConfigFromForm();
	}

	function updateDecisionCondition(index: number, key: string, operator: string, value?: string) {
		// XSS 対策: key と value をサニタイズ
		const sanitizedKey = sanitizeInput(key, 100);
		const sanitizedValue =
			value !== undefined && value !== '' ? sanitizeInput(value, 200) : undefined;
	...
	function updateSwitchCase(index: number, field: keyof SwitchCase, value: unknown) {
		...
		switchCases = switchCases.map((c, i) => (i === index ? { ...c, [field]: sanitizedValue } : c));
		updateConfigFromForm();
	}

	function updateSwitchCaseValues(index: number, valuesStr: string) {
		// XSS 対策: values をサニタイズ
		const sanitizedStr = sanitizeInput(valuesStr, 500);

		// Parse comma-separated values
		const values = sanitizedStr
			.split(',')
			.map((v) => v.trim())
			.filter((v) => v !== '');
