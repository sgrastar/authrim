<script lang="ts">
	...
	const label = $derived(data?.label || 'Authentication');

	// Support both single method and array of methods
	const methodsDisplay = $derived(() => {
...
<div class="auth-node" class:selected>
	...
	<div class="node-body">
		<span class="config-value">{methodsDisplay()}</span>
	</div>
	...
	<div class="handle-wrapper handle-unavailable">
		<Handle type="source" position={Position.Right} id="unavailable" style="top: 80%;" />
		<span class="handle-label unavailable">N/A</span>
	</div>
</div>
...
<style>
	.auth-node {
		...
		border-left: 3px solid var(--flow-node-color-check-alt);
	...
	.handle-wrapper.handle-unavailable {
		right: -8px;
		top: 70%;
		transform: translateY(-50%);
	}
