<script lang="ts">
	...
	async function loadProfile() {
		...
		try {
			...
			protocolScope = profile.protocolScope;
			audienceMode = profile.audienceMode;
			usage = profile.usage.join(',');
			issuerEntityId = profile.issuerEntityId ?? '';
	...
	async function saveProfile() {
		...
		errorMessage = null;
		try {
			await adminIdentityMappingAPI.updatePersistentIdentifierProfile(profileId, {
				displayName,
				profileKey,
	...
	async function deleteProfile() {
		if (!window.confirm($LL.admin_identity_mapping_persistent_delete_confirm())) return;
		deleting = true;
		errorMessage = null;
		try {
			...
			errorMessage =
				error instanceof Error
					? error.message
					: $LL.admin_identity_mapping_persistent_delete_failed();
		} finally {
			deleting = false;
		}
	}
...
<svelte:head>
	<title>{$LL.admin_identity_mapping_persistent_edit_title()}</title>
</svelte:head>

<AdminPageShell width="narrow">
	<AdminPageHeader
		eyebrow={$LL.admin_identity_mapping_title()}
		title={$LL.admin_identity_mapping_persistent_edit_title()}
		description={$LL.admin_identity_mapping_persistent_description()}
	>
		{#snippet actions()}
			<button class="danger" type="button" onclick={deleteProfile} disabled={loading || deleting}>
				{$LL.admin_identity_mapping_persistent_delete()}
			</button>
		{/snippet}
	</AdminPageHeader>
		...
		{:else}
			...
			>
				...
				<div class="form-actions">
					<a href="/admin/field-mapping/persistent-identifiers"
						>{$LL.admin_identity_mapping_back()}</a
					>
