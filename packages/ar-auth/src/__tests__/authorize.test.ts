...
const mockFindConsentGateReceipt = vi.hoisted(() => vi.fn());
const mockConsumeConsentGateReceipt = vi.hoisted(() => vi.fn());
const mockIsConsentGateEnabled = vi.hoisted(() => vi.fn());
vi.mock('@authrim/ar-lib-core', async () => {
  ...
  return {
    ...
    isTenantFlowProtocolConsentGatesEnabled: mockIsConsentGateEnabled,
    ConsentGateDecisionReceiptRepository: class {
      findById(...args: unknown[]) {
        return mockFindConsentGateReceipt(...args);
      }
      consume(...args: unknown[]) {
        return mockConsumeConsentGateReceipt(...args);
      }
    },
  };
});
...
describe('Authorization Handler', () => {
  ...
  beforeEach(() => {
    mockFindConsentGateReceipt.mockReset();
    mockConsumeConsentGateReceipt.mockReset();
    mockIsConsentGateEnabled.mockReset();
    mockIsConsentGateEnabled.mockResolvedValue(true);
    vi.clearAllMocks();
  ...
  describe('Edge Cases', () => {
    ...
    it('uses the Flow receipt scope and claim subsets as the authorization code grant', async () => {
      seedSession(env);
      const selectedHashBytes = await crypto.subtle.digest(
        'SHA-256',
        new TextEncoder().encode(JSON.stringify({ scopes: ['openid'], claims: ['email'] }))
      );
      const selectedHash = btoa(String.fromCharCode(...new Uint8Array(selectedHashBytes)))
        .replace(/\+/gu, '-')
        .replace(/\//gu, '_')
        .replace(/=+$/gu, '');
      getChallengeMap(env).set('confirm_flow_consent', {
        id: 'confirm_flow_consent',
        tenantId: 'default',
        type: 'consent',
        userId: 'test-user',
        challenge: 'confirm_flow_consent',
        metadata: {
          purpose: 'authorize_consent_confirmation',
      ...
      const receipt = {
        ...
        flow_id: 'flow_1',
        flow_version_id: 'flow_version_1',
        flow_node_id: 'oidc_consent',
        gate_kind: 'oidc_authorization',
        subject_user_id: 'test-user',
        target_type: 'oidc_client',
        target_id: 'test-client',
        policy_id: null,
        protocol_request_id: 'login_challenge_1',
        ...
        decision: {
          ...
          release: {
            ...
            requested_claims: ['email', 'phone_number'],
            selected_claims: ['email'],
            required_claims: [],
          },
        },
        evidence_record_ids: ['evidence_1'],
        state: 'ready',
      ...
      mockFindConsentGateReceipt.mockResolvedValueOnce(receipt);
      mockConsumeConsentGateReceipt.mockResolvedValueOnce({ ...receipt, state: 'consumed' });
      const authCodeStore = getAuthCodeStore(env);
      const claims = encodeURIComponent(
        JSON.stringify({ userinfo: { email: null, phone_number: null } })
      );

      const response = await app.request(
        `/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/callback&scope=openid%20profile&claims=${claims}&state=flow-consent-state&_consent_confirmation_challenge=confirm_flow_consent`,
        {
          method: 'GET',
      ...
      expect(authCodeStore.storeCodeRpc).toHaveBeenCalledWith(
        expect.objectContaining({
          scope: 'openid',
          claims: JSON.stringify({ userinfo: { email: null }, id_token: {} }),
          state: 'flow-consent-state',
        })
      );
      expect(mockConsumeConsentGateReceipt).toHaveBeenCalledWith(
        expect.objectContaining({
          target_id: 'test-client',
          protocol_request_id: 'login_challenge_1',
          release_set_hash: selectedHash,
    ...
    it('rejects an issued Flow receipt after the tenant Consent Gate flag is disabled', async () => {
      seedSession(env);
      mockIsConsentGateEnabled.mockResolvedValue(false);
      getChallengeMap(env).set('confirm_disabled_flow_consent', {
        id: 'confirm_disabled_flow_consent',
        tenantId: 'default',
        type: 'consent',
        userId: 'test-user',
        challenge: 'confirm_disabled_flow_consent',
        metadata: {
          purpose: 'authorize_consent_confirmation',
          consent_gate_receipt_id: 'cgr_0123456789abcdef0123456789abcdef',
          consent_gate_protocol_request_id: 'login_challenge_1',
      ...
      const response = await app.request(
        '/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/callback&scope=openid&state=disabled-flow-consent-state&_consent_confirmation_challenge=confirm_disabled_flow_consent',
        {
          method: 'GET',
          headers: { Cookie: `authrim_session=${encodeURIComponent(TEST_SESSION_ID)}` },
        },
      ...
      const redirectUrl = new URL(response.headers.get('Location')!);
      expect(redirectUrl.searchParams.get('error')).toBe('temporarily_unavailable');
      expect(mockFindConsentGateReceipt).not.toHaveBeenCalled();
      expect(mockConsumeConsentGateReceipt).not.toHaveBeenCalled();
    });

    it('rejects a Flow receipt bound to another OIDC client', async () => {
      seedSession(env);
      getChallengeMap(env).set('confirm_wrong_client', {
        id: 'confirm_wrong_client',
        tenantId: 'default',
        type: 'consent',
        userId: 'test-user',
        challenge: 'confirm_wrong_client',
        metadata: {
          purpose: 'authorize_consent_confirmation',
          consent_gate_receipt_id: 'cgr_0123456789abcdef0123456789abcdef',
          consent_gate_protocol_request_id: 'login_challenge_1',
      ...
      mockFindConsentGateReceipt.mockResolvedValueOnce({
        id: 'cgr_0123456789abcdef0123456789abcdef',
        gate_kind: 'oidc_authorization',
        subject_user_id: 'test-user',
        target_type: 'oidc_client',
        target_id: 'another-client',
        protocol_request_id: 'login_challenge_1',
        decision: { release: { protocol: 'oidc' } },
      });

      const response = await app.request(
        '/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/callback&scope=openid&_consent_confirmation_challenge=confirm_wrong_client',
        {
          method: 'GET',
          headers: { Cookie: `authrim_session=${encodeURIComponent(TEST_SESSION_ID)}` },
        },
    ...
    it('rejects a SAML Flow receipt on the OIDC authorization continuation', async () => {
      seedSession(env);
      getChallengeMap(env).set('confirm_cross_protocol', {
        id: 'confirm_cross_protocol',
        tenantId: 'default',
        type: 'consent',
        userId: 'test-user',
        challenge: 'confirm_cross_protocol',
        metadata: {
          purpose: 'authorize_consent_confirmation',
          consent_gate_receipt_id: 'cgr_0123456789abcdef0123456789abcdef',
          consent_gate_protocol_request_id: 'login_challenge_1',
      ...
      mockFindConsentGateReceipt.mockResolvedValueOnce({
        ...
        protocol_request_id: 'login_challenge_1',
        decision: { release: { protocol: 'saml' } },
      });

      const response = await app.request(
        '/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/callback&scope=openid&_consent_confirmation_challenge=confirm_cross_protocol',
        {
          method: 'GET',
          headers: { Cookie: `authrim_session=${encodeURIComponent(TEST_SESSION_ID)}` },
        },
    ...
    it('rejects a Flow receipt that omits the protocol-required openid scope', async () => {
      seedSession(env);
      getChallengeMap(env).set('confirm_missing_required_scope', {
        id: 'confirm_missing_required_scope',
        tenantId: 'default',
        type: 'consent',
        userId: 'test-user',
        challenge: 'confirm_missing_required_scope',
        metadata: {
          purpose: 'authorize_consent_confirmation',
          consent_gate_receipt_id: 'cgr_0123456789abcdef0123456789abcdef',
          consent_gate_protocol_request_id: 'login_challenge_1',
      ...
      const response = await app.request(
        '/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/callback&scope=openid&_consent_confirmation_challenge=confirm_missing_required_scope',
        {
          method: 'GET',
          headers: { Cookie: `authrim_session=${encodeURIComponent(TEST_SESSION_ID)}` },
        },
