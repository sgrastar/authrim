 ...
 */
export const COUNTRY_TO_PARTITION = {
 ...
 */
export function clearPartitionSettingsCache() {
...
export class PIIPartitionRouter {
     ...
     */
    constructor(coreAdapter, defaultPiiAdapter, kvNamespace) {
     ...
     */
    registerPartition(partition, adapter) {
     ...
     */
    getAdapterForPartition(partition) {
     ...
     */
    async resolvePartitionForUser(userId) {
        const user = await this.coreAdapter.queryOne('SELECT pii_partition FROM users_core WHERE id = ?', [userId]);
     ...
     */
    async resolvePartitionForNewUser(tenantId, attributes, cfData, settings) {
     ...
     */
    async getPartitionSettings(tenantId) {
        ...
        if (this.kvNamespace) {
            ...
            settings = await this.kvNamespace.get(kvKey, { type: 'json' });
        }
     ...
     */
    async savePartitionSettings(tenantId, settings) {
        if (!this.kvNamespace) {
     ...
     */
    async getPartitionStats(tenantId) {
        const sql = tenantId
        ...
        const results = await this.coreAdapter.query(sql, params);
        const stats = new Map();
 ...
 */
export function buildPartitionSettingsKvKey(tenantId) {
 ...
 */
export function getDefaultPartitionSettings(availablePartitions = [DEFAULT_PARTITION]) {
    return {
 ...
 */
export function validatePartitionSettings(settings, availablePartitions) {
 ...
 */
export function createPIIPartitionRouter(coreAdapter, defaultPiiAdapter, additionalPartitions, kvNamespace) {
