...
import { createOAuthConfigManager } from '../utils/oauth-config';
 ...
 */
export class AuthorizationCodeStore extends DurableObject {
    codes = new Map();
    cleanupInterval = null;
    initialized = false;
    configManager;
    actorCtx;
    log = createLogger().module('AuthorizationCodeStore');
    // User code counter for O(1) DDoS protection check (instead of O(n) scan)
    // Maps userId to count of active (non-expired) codes
    userCodeCounts = new Map();
    // Configuration (loaded from KV > env > default in initializeState)
    CODE_TTL; // Default: 60 seconds per OAuth 2.0 Security BCP
    CLEANUP_INTERVAL_MS; // Default: 30 seconds
    MAX_CODES_PER_USER; // DDoS protection
    constructor(ctx, env) {
        super(ctx, env);
    ...
    async initializeStateBlocking() {
        ...
        try {
            ...
            this.log.warn('Failed to load config from KV, using env/default values', {}, error);
            // Keep constructor-initialized values (from env)
        }
        try {
            // Load all codes from individual key storage (code:*)
            const storedCodes = await this.actorCtx.storage.list({
     ...
     */
    async generateCodeChallenge(verifier, method = 'S256') {
        if (method === 'plain') {
            return verifier;
        }
    ...
    async consumeCode(request) {
        ...
        if (!stored) {
            ...
            const fromStorage = await this.actorCtx.storage.get(this.buildCodeKey(request.code));
            if (fromStorage) {
                stored = fromStorage;
        ...
        if (stored.codeChallenge) {
            ...
            const challenge = await this.generateCodeChallenge(request.codeVerifier, stored.codeChallengeMethod || 'S256');
     ...
     */
    async registerIssuedTokens(code, accessTokenJti, refreshTokenJti) {
        await this.initializeState();
        const stored = this.codes.get(code);
        if (!stored) {
        ...
        try {
            ...
            if (path === '/code/consume' && request.method === 'POST') {
                ...
                try {
                    ...
                    this.log.warn('consumeCode error', {}, error);
            ...
            if (path.startsWith('/code/') && path.endsWith('/tokens') && request.method === 'POST') {
                ...
                const success = await this.registerIssuedTokens(code, body.accessTokenJti, body.refreshTokenJti);
            ...
            if (path === '/reload-config' && request.method === 'POST') {
                ...
                try {
                    ...
                    this.log.error('Failed to reload config', {}, error);
                    return new Response(JSON.stringify({
                        status: 'error',
