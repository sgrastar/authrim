...
export class ChallengeStore extends DurableObject {
    ...
    log = createLogger().module('ChallengeStore');
    // Configuration
     ...
     */
    async consumeChallengeRpc(request) {
        return this.consumeChallenge(request);
    }
    /**
     * RPC: Delete a challenge
     */
    async deleteChallengeRpc(id) {
    ...
    async getStatusRpc() {
        ...
        const storedChallenges = await this.ctx.storage.list({
    ...
    async cleanupExpiredChallenges() {
        ...
        if (cleaned > 0) {
            this.log.info('Cleaned up expired/consumed challenges', { count: cleaned });
        }
    }
    /**
    ...
    async getChallenge(id) {
        ...
        // 2. If not in cache, check Durable Storage
        if (!challenge) {
            const storedChallenge = await this.ctx.storage.get(this.buildChallengeKey(id));
            if (storedChallenge) {
                challenge = storedChallenge;
                // Promote to cache
    ...
    async fetch(request) {
        ...
        try {
            ...
            if (path === '/challenge' && request.method === 'POST') {
                ...
                await this.storeChallenge(body);
            ...
            if (path === '/challenge/consume' && request.method === 'POST') {
                ...
                try {
                    const result = await this.consumeChallenge(body);
                    return new Response(JSON.stringify(result), {
                        headers: { 'Content-Type': 'application/json' },
                    });
                }
                catch (error) {
                    this.log.warn('consumeChallenge error', {}, error);
