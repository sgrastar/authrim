...
export class KeyManager extends DurableObject {
    keyManagerState = null;
    ecKeyManagerState = null;
    constructor(ctx, env) {
        super(ctx, env);
    ...
    async initializeStateBlocking() {
        ...
        const storedECState = await this.ctx.storage.get('ecState');
        if (storedECState) {
     ...
     */
    async getActiveECKeyRpc(algorithm) {
    ...
    async migrateIsActiveToStatus() {
        ...
        for (const key of state.keys) {
            if ('isActive' in key && !('status' in key)) {
                ...
                key.status = key.isActive ? 'active' : 'overlap';
                // @ts-expect-error - migration from old schema
                delete key.isActive;
            }
        }
     ...
     */
    async emergencyRotateKeys(reason) {
        await this.initializeState();
        const state = this.getState();
        const now = Date.now();
     ...
     */
    async generateNewECKey(algorithm) {
        await this.initializeECState();
     ...
     */
    async getActiveECKey(algorithm) {
        await this.initializeECState();
        const state = this.getECState();
     ...
     * @returns Object with old and new key IDs
     */
    async emergencyRotateECKeys(algorithm, reason) {
        await this.initializeECState();
        const state = this.getECState();
        const now = Date.now();
    ...
    async fetch(request) {
        ...
        try {
            ...
            if (path.startsWith('/ec/active/') && request.method === 'GET') {
                ...
                const activeKey = await this.getActiveECKey(algorithm);
                if (!activeKey) {
                    return new Response(JSON.stringify({ error: `No active EC key found for ${algorithm}` }), {
                        status: 404,
                        headers: { 'Content-Type': 'application/json' },
                    });
                }
