 ...
 */
export async function processAuditQueue(batch, env, logger) {
...
async function processMessage(message, env, logger) {
    ...
    if (type === 'event_log') {
        ...
        const adapter = createAuditPrimaryStorageAdapter(env, resolved.auditProfile.primary, 'event', { id: `queue-event:${resolved.auditProfile.id}` });
        if (!adapter) {
            throw new Error(`audit_queue_primary_unresolved:${resolved.auditProfile.id}:event`);
        }
        try {
            const result = await adapter.writeEventLogBatch(entries);
            if (!result.success) {
    ...
    if (type === 'pii_log') {
        ...
        const adapter = createAuditPrimaryStorageAdapter(env, resolved.auditProfile.primary, 'pii', { id: `queue-pii:${resolved.auditProfile.id}` });
        if (!adapter) {
            throw new Error(`audit_queue_primary_unresolved:${resolved.auditProfile.id}:pii`);
        }
        try {
            const result = await adapter.writePIILogBatch(entries);
            if (!result.success) {
...
function getR2BucketBinding(env, bucketRef) {
    const binding = env[bucketRef];
    return binding && typeof binding === 'object' ? binding : null;
}
async function writeArchiveTarget(target, body, env) {
    if (target.type !== 'r2') {
    ...
    if (body.type === 'event_log') {
        const result = await adapter.writeEventLogBatch(body.entries);
        if (!result.success) {
            throw new Error(result.errorMessage ?? 'archive_write_failed');
        }
        return;
    }
    const result = await adapter.writePIILogBatch(body.entries);
    if (!result.success) {
        throw new Error(result.errorMessage ?? 'archive_write_failed');
    }
}
function emitLogpushSink(target, body) {
    for (const entry of body.entries) {
        console.log(JSON.stringify(buildCanonicalAuditRecord(target, body, entry, 'logpush')));
    }
}
async function deliverHttpSink(target, body, env) {
    ...
    const authToken = target.authTokenRef != null
        ? env[target.authTokenRef]
        : undefined;
    const headers = {
...
async function deliverSinkTarget(target, body, env) {
...
async function processFanoutMessage(body, env, logger) {
    const fanout = body.fanout;
 ...
 */
async function batchUpsertEventLog(db, entries) {
    if (entries.length === 0) {
        return;
    }
    const adapter = ensureDatabaseAdapter(db, 'audit-queue-event');
    await adapter.batch(entries.map((e) => ({
 ...
 */
async function batchUpsertPIILog(db, entries) {
    if (entries.length === 0) {
        return;
    }
 ...
 * @param logger - Logger instance (optional)
 */
export async function processDLQQueue(batch, env, logger) {
    ...
    for (const message of batch.messages) {
        try {
            ...
            if (env.AUDIT_ARCHIVE) {
                ...
                await env.AUDIT_ARCHIVE.put(r2Key, JSON.stringify({
                    messageId: message.id,
 ...
 */
export async function cleanupExpiredEventLogs(db, tenantId, batchSize = 1000) {
    const now = Date.now();
 ...
 * @returns Number of entries deleted
 */
export async function cleanupExpiredPIILogs(db, tenantId, batchSize = 1000) {
    const now = Date.now();
