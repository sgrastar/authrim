...
function targetDatasetForBinding(bindingRef) {
...
export function targetToBackendId(target) {
    if (!target) {
        return null;
    }
    if (target.type === 'd1') {
...
export function auditTargetFromBackendConfig(backend) {
    if (!backend.enabled) {
        return null;
    }
    if (backend.type === 'D1') {
...
export function buildAuditStorageBackendsFromProfile(profile, baseBackends = DEFAULT_AUDIT_STORAGE_CONFIG.backends) {
    const backends = [...baseBackends];
    const seen = new Set(backends.map((backend) => backend.id));
    const maybePush = (backend) => {
...
export function buildPrimaryBackendMap(profile, requestedBackends) {
    const mapping = new Map([['archive-only', null]]);
    if (profile.primary?.type === 'd1') {
        const logicalD1Primary = {
            type: 'd1',
            bindingRef: 'DB',
            dataset: 'event_log',
        };
...
export function buildAuditStorageConfigFromProfile(profile, options = {}) {
    ...
    const retentionConfig = options.retentionConfig ?? {
        eventLogRetentionDays: profileRetention?.eventLogRetentionDays ??
            profileRetention?.primaryDays ??
            DEFAULT_AUDIT_STORAGE_CONFIG.defaultRetention.eventLogRetentionDays,
        piiLogRetentionDays: profileRetention?.piiLogRetentionDays ??
            profileRetention?.primaryDays ??
            DEFAULT_AUDIT_STORAGE_CONFIG.defaultRetention.piiLogRetentionDays,
        archiveBeforeDelete: profileRetention?.archiveBeforeDelete ??
