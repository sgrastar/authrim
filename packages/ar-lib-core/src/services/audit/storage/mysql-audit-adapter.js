import { openMysqlConnection, } from '../../../db/adapters/mysql-connection';
...
export class MysqlAuditAdapter {
    id;
    hyperdrive;
    schema;
    isPiiDb;
    ...
    async writeEventLogBatch(entries) {
        ...
        try {
            ...
            entries.forEach((entry) => {
                values.push(entry.id, entry.tenantId, entry.eventType, entry.eventCategory, entry.result, entry.severity, entry.errorCode ?? null, entry.errorMessage ?? null, entry.anonymizedUserId ?? null, entry.clientId ?? null, entry.sessionId ?? null, entry.requestId ?? null, entry.durationMs ?? null, entry.detailsR2Key ?? null, entry.detailsJson ?? null, entry.retentionUntil ?? null, entry.createdAt);
            });
            await client.execute(buildInsertIfNotExistsSql(qualifyTable(this.schema, 'event_log'), [...columns], 'id', entries.length), values);
            return {
                success: true,
    ...
    async writePIILogBatch(entries) {
        ...
        try {
            ...
            entries.forEach((entry) => {
                values.push(entry.id, entry.tenantId, entry.userId, entry.anonymizedUserId, entry.changeType, entry.affectedFields, entry.valuesR2Key ?? null, entry.valuesEncrypted ?? null, entry.encryptionKeyId, entry.encryptionIv, entry.actorUserId ?? null, entry.actorType, entry.requestId ?? null, entry.legalBasis ?? null, entry.consentReference ?? null, entry.retentionUntil, entry.createdAt);
            });
            await client.execute(buildInsertIfNotExistsSql(qualifyTable(this.schema, 'pii_log'), [...columns], 'id', entries.length), values);
            return {
                success: true,
                entriesWritten: entries.length,
                backend: this.id,
    ...
    async queryEventLog(client, options, startTime) {
        ...
        const result = await client.query(`${sql} LIMIT ? OFFSET ?`, [...params, limit + 1, offset]);
        ...
        if (hasMore || offset > 0) {
            const countResult = await client.query(`SELECT COUNT(*) as count FROM ${qualifyTable(this.schema, 'event_log')} WHERE ${this.buildWhereClause(options, []).conditions.join(' AND ')}`, params);
    ...
    async queryPIILog(client, options, startTime) {
        ...
        let totalCount = entries.length + offset;
        if (hasMore || offset > 0) {
            const countResult = await client.query(`SELECT COUNT(*) as count FROM ${qualifyTable(this.schema, 'pii_log')} WHERE ${this.buildWhereClause(options, []).conditions.join(' AND ')}`, params);
            totalCount = Number(countResult.rows[0]?.count ?? 0);
        }
        return {
            piiEntries: entries,
    ...
    async count(options) {
        try {
            ...
            const result = await client.query(`SELECT COUNT(*) as count FROM ${qualifyTable(this.schema, table)} WHERE ${conditions.join(' AND ')}`, params);
    ...
    async listRetentionCandidates(logType, beforeTime, tenantId, batchSize = 1000) {
        try {
            ...
            const result = await client.query(`SELECT ${selectColumns}
         FROM ${table}
         WHERE ${where}
    ...
    async deleteByRetention(logType, beforeTime, tenantId, batchSize = 1000) {
        try {
            ...
            const result = await client.execute(`DELETE target
         FROM ${table} target
         INNER JOIN (
           SELECT id
           FROM ${table}
...
export function createMysqlAuditAdapter(hyperdrive, options) {
