...
export class R2AuditAdapter {
    ...
    async writeEventLogBatchJsonl(entries, startTime) {
    ...
    async writePIILogBatchJsonl(entries, startTime) {
        const grouped = this.groupEntriesByHour(entries, (e) => ({
            tenantId: e.tenantId,
            createdAt: e.createdAt,
        }));
    ...
    async query(options) {
        ...
        try {
            ...
            const filteredObjects = this.filterObjectsByDate(listed.objects, options.startTime, options.endTime);
    ...
    async listRetentionCandidates(_logType, _beforeTime, _tenantId, _batchSize = 100) {
        return [];
    }
    async deleteByRetention(logType, beforeTime, tenantId, batchSize = 100) {
    ...
    // Helper Methods
    // ---------------------------------------------------------------------------
    buildEntryKey(logType, tenantId, entryId, createdAt) {
        const date = new Date(createdAt);
        const dateStr = date.toISOString().slice(0, 10); // YYYY-MM-DD
        return `${this.pathPrefix}/${logType}/${tenantId}/${dateStr}/${entryId}.json`;
    }
    groupEntriesByHour(entries, getInfo) {
    ...
    filterObjectsByDate(objects, startTime, endTime) {
        if (!startTime && !endTime)
            return objects;
        return objects.filter((obj) => {
            const dateMatch = obj.key.match(/\/(\d{4}-\d{2}-\d{2})\//);
            if (!dateMatch)
                return true; // Include if no date in path
            const objDate = new Date(dateMatch[1]).getTime();
            if (startTime && objDate < startTime - 86400000)
                return false; // Day buffer
            if (endTime && objDate > endTime + 86400000)
                return false;
            return true;
        });
    }
    matchesQueryOptions(entry, options) {
        // Time range filter
        if (options.startTime && entry.createdAt < options.startTime)
            return false;
        if (options.endTime && entry.createdAt >= options.endTime)
            return false;
        // Common filters
        if (options.anonymizedUserId && 'anonymizedUserId' in entry) {
            if (entry.anonymizedUserId !== options.anonymizedUserId)
                return false;
        }
        if (options.requestId && 'requestId' in entry) {
            if (entry.requestId !== options.requestId)
                return false;
        }
        // Event log filters
        if (options.logType === 'event' && 'eventType' in entry) {
            const eventEntry = entry;
            if (options.eventType && eventEntry.eventType !== options.eventType)
                return false;
            if (options.eventCategory && eventEntry.eventCategory !== options.eventCategory)
                return false;
            if (options.result && eventEntry.result !== options.result)
                return false;
            if (options.clientId && eventEntry.clientId !== options.clientId)
                return false;
        }
        // PII log filters
        if (options.logType === 'pii' && 'userId' in entry) {
            const piiEntry = entry;
            if (options.userId && piiEntry.userId !== options.userId)
                return false;
            if (options.changeType && piiEntry.changeType !== options.changeType)
                return false;
        }
        return true;
    }
 ...
 */
export function createR2AuditAdapter(bucket, options) {
