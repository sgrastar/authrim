...
export function auditRoutingRuleMatches(rule, context) {
 ...
 */
export function resolveAuditRoutingTargets(config, context) {
    const sortedRules = [...config.routingRules]
        .filter((rule) => rule.enabled)
        .sort((a, b) => a.priority - b.priority);
    const resolved = {
        primaryStore: context.logType === 'event' ? config.defaultEventBackend : config.defaultPiiBackend,
        archiveStores: [],
        forwardingSinks: [],
        retention: {},
        matchedRuleNames: [],
