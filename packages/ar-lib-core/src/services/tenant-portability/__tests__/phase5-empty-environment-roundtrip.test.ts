...
import { createPhase5SqliteInspectionPolicies } from '../phase5-sqlite-references';
import {
...
function phase5Plan(): PlannedInstalledSqliteDataset[] {
  const plans = new Map(
  ...
  const missing = PHASE5_CUMULATIVE_SQLITE_DATASET_REGISTRATIONS.flatMap((registration) => {
    const entry = plans
      .get(registration.family as 'core' | 'admin')
      ?.entries.find((candidate) => candidate.table === registration.table);
    return entry?.capture ? [] : [`${registration.dataset.id}:${entry?.concerns.join(',')}`];
  });
  if (missing.length) throw new Error(`missing_capture:${missing.join(';')}`);
...
function ordered(planned: readonly PlannedInstalledSqliteDataset[]) {
  const policies = createPhase5SqliteInspectionPolicies(planned, {
  ...
  it('restores every dataset contract and target-encrypts representative protocol secrets', async () => {
    ...
    try {
      ...
      sourceCore.exec(`
        ...
        INSERT INTO flows(
          id,tenant_id,profile_id,name,graph_definition,created_at,updated_at,
          slug,display_name,kind,status,published_version_id
        ) VALUES(
          'flow-a','tenant-a','human-basic','Tenant Login','{}',1,1,
          'tenant-login','Tenant Login','login','published','flow-version-a'
        );
        INSERT INTO flow_versions(
          ...
          'flow-version-a','tenant-a','flow-a',1,'authrim.login_ui.contract.v1','{}',
          '{"valid":true}',1,1
        );
        INSERT INTO flow_assignments(
          id,tenant_id,target_type,target_id,flow_kind,flow_id,enabled,created_at,updated_at
        ) VALUES('assignment-a','tenant-a','tenant',NULL,'login','flow-a',1,1,1);
      `);
      ...
      for (const entry of planned) {
        ...
        const rows = source
          ...
          .all('phase5-snapshot', 'tenant-a', '', 10_000)
      ...
      const key = await createTenantBundleKeyEnvelope('phase5-roundtrip-password');
      ...
      for (const policy of ordered(planned)) {
        ...
        for (const rowJson of restored.get(entry.dataset.id) ?? []) {
          ...
          if (entry.dataset.id === 'core.flows') fields.published_version_id = ['null', null];
      ...
      targetCore
        .prepare('UPDATE flows SET published_version_id = ? WHERE tenant_id = ? AND id = ?')
        .run('flow-version-a', 'tenant-a', 'flow-a');

      expect(restored.size).toBe(datasets.length);
      expect(planned).toHaveLength(PHASE5_CUMULATIVE_SQLITE_DATASET_REGISTRATIONS.length);
      expect(
      ...
      expect(
        targetCore
          .prepare(
            `SELECT f.published_version_id, v.flow_id
               FROM flows f JOIN flow_versions v ON v.id = f.published_version_id
              WHERE f.tenant_id = 'tenant-a' AND f.id = 'flow-a'`
          )
          .get()
      ).toEqual({ published_version_id: 'flow-version-a', flow_id: 'flow-a' });
      expect(
        targetCore.prepare("SELECT flow_id FROM flow_assignments WHERE id = 'assignment-a'").get()
      ).toEqual({ flow_id: 'flow-a' });
      await expect(
        decodeKeyManagerTenantBackupRow(
