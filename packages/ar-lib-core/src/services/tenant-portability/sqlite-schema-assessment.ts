import { readSnapshotUniqueTerms, type SnapshotUniqueTerm } from './sqlite-index-expression';
import type { BackupSchemaTable } from './sqlite-schema-types.js';
...
export function assessSnapshotTable(
  ...
  const result = assessSqliteCaptureStructure(table);
  const concerns = [...result.concerns];
  if (!table.columns.some((column) => column.name === tenantColumn && !column.generated)) {
    concerns.push('ownership_adapter_required');
  }
  return {
    concerns,
    schema: concerns.length || !result.schema ? null : { ...result.schema, tenantColumn },
  };
}

/** Structural metadata only: does not infer or grant tenant ownership. */
export function assessSqliteCaptureStructure(table: BackupSchemaTable): {
  concerns: SnapshotSchemaConcern[];
  schema: Omit<SnapshotTableSchema, 'tenantColumn'> | null;
} {
  ...
  const uniqueExpressions: SnapshotUniqueTerm[][] = [];
  for (const index of table.indexes.filter((candidate) => candidate.unique)) {
    const keys = index.columns.filter((column) => column.key);
    if (keys.some((column) => column.name === null)) {
      const terms = readSnapshotUniqueTerms(
        table.name,
        table.columns.map((column) => column.name),
        index
      );
      if (terms) uniqueExpressions.push(terms);
      else concerns.add('expression_unique_index');
    }
    ...
    schema:
      concerns.size > 0
        ...
        : {
            ...
            uniqueKeys,
            ...(uniqueExpressions.length ? { uniqueExpressions } : {}),
          },
  };
}
