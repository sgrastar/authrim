...
async function resolveConsentPolicyRequirements(
  adapter: DatabaseAdapter,
  tenantId: string,
  clientId: string | null,
  statements: ConsentStatement[],
  context: ConsentRequirementResolutionContext = {}
): Promise<ResolvedConsentRequirement[] | null> {
  const targetType = context.target_type ?? 'oidc_client';
  const targetId = context.target_id ?? clientId ?? '';
  let assignmentRows: Array<{ policy_id: string }>;
  try {
    assignmentRows = await adapter.query<{
      policy_id: string;
    }>(
      `SELECT policy_id
  ...
  const items = await adapter.query<{
    ...
    `SELECT statement_id, requirement, version_mode, version_id, min_version,
            checkbox_mode, checkbox_default_checked, binding_type, binding_value,
            evidence_profile, language_fallback, display_order
       FROM consent_policy_items
      WHERE tenant_id = ? AND policy_id = ?
      ORDER BY display_order ASC, created_at ASC`,
    [tenantId, assignmentRows[0].policy_id]
  );

  const results: ResolvedConsentRequirement[] = [];

  for (const item of items) {
    if (item.requirement === 'hidden') continue;
    if (!policyItemMatchesRequestContext(item, context)) continue;
    const statement = statements.find((candidate) => candidate.id === item.statement_id);
    if (!statement) continue;

    const currentVersion = await getCurrentVersion(adapter, tenantId, statement.id);
    if (!currentVersion) continue;

    let displayVersion = currentVersion;
    let minVersion: string | undefined;
    let fixedVersionId: string | undefined;

    if (item.version_mode === 'fixed' && item.version_id) {
      const fixedVersion = await getVersionById(adapter, tenantId, statement.id, item.version_id);
      if (!fixedVersion) continue;
      displayVersion = fixedVersion;
      minVersion = fixedVersion.version;
      fixedVersionId = fixedVersion.id;
    } else if (item.version_mode === 'minimum' && item.min_version) {
      minVersion = item.min_version;
    } else if (statement.reconsent_on_version_change !== false) {
    ...
    results.push({
      ...
      reconsent_interval_days: statement.reconsent_interval_days,
      enforcement: 'block' as ConsentEnforcement,
      show_deletion_link: false,
      checkbox_mode: item.checkbox_mode as ResolvedConsentRequirement['checkbox_mode'],
      checkbox_default_checked: item.checkbox_default_checked === 1,
      binding_type: item.binding_type
        ? (item.binding_type as ResolvedConsentRequirement['binding_type'])
        : undefined,
      binding_value: item.binding_value ?? undefined,
      evidence_profile: item.evidence_profile ?? undefined,
      language_fallback: item.language_fallback ?? undefined,
...
function policyItemMatchesRequestContext(
  item: {
    binding_type: string | null;
    binding_value: string | null;
  },
  ...
  if (!item.binding_type || !item.binding_value) return true;
  const values = splitBindingValues(item.binding_value);
  ...
  switch (item.binding_type) {
    ...
    case 'claim':
      return intersects(values, context.requested_claims ?? []);
    case 'saml_attribute':
      return intersects(values, context.requested_saml_attributes ?? []);
    case 'destination_field_set':
      return intersects(values, context.requested_destination_field_sets ?? []);
    default:
      return true;
  }
}
...
export async function resolveConsentRequirements(
  ...
  const policyRequirements = await resolveConsentPolicyRequirements(
    adapter,
    tenantId,
    clientId,
    statements,
  ...
  if (policyRequirements) return policyRequirements;
