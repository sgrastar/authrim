...
export async function hashPassword(password) {
    ...
    }, passwordKey, PASSWORD_HASH_CONFIG.KEY_LENGTH * 8 // bits
    );
 ...
 */
async function verifyPbkdf2Password(password, hash) {
    ...
    const derivedBits = await crypto.subtle.deriveBits({
        ...
        hash: PASSWORD_HASH_CONFIG.HASH,
    }, passwordKey, PASSWORD_HASH_CONFIG.KEY_LENGTH * 8);
 ...
 */
async function verifyLegacySha256Password(password, hash) {
    const encoder = new TextEncoder();
 ...
 */
export function passwordHashNeedsUpgrade(hash) {
 ...
 * ```
 */
export async function verifyClientSecretHash(providedSecret, storedHash) {
