 ...
 * );
 */
export async function retryD1Operation(operation, operationName, config = {}) {
    ...
    for (let attempt = 0; attempt <= cfg.maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(cfg.initialDelayMs * Math.pow(cfg.backoffMultiplier, attempt), cfg.maxDelayMs);
    ...
    log.error(`${operationName}: All ${cfg.maxRetries + 1} attempts failed`, {
        operationName,
    }, lastError ?? undefined);
 ...
 * );
 */
export async function retryD1Batch(operations, operationName, config = {}) {
    return retryD1Operation(async () => {
