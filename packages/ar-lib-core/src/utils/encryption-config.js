 ...
 */
export const ENCRYPTION_CONFIG_METADATA = {
    PII_ENCRYPTION_ENABLED: {
        type: 'boolean',
        label: 'PII Encryption Enabled',
        description: 'Enable encryption for PII fields. Requires PII_ENCRYPTION_KEY environment variable to be set.',
    },
    PII_ENCRYPTION_ALGORITHM: {
        type: 'select',
        label: 'Encryption Algorithm',
        description: 'Algorithm for PII encryption. AES-256-GCM (recommended) provides authenticated encryption.',
 ...
 */
function parseAlgorithm(value, defaultValue) {
    if (!value)
        return defaultValue;
 ...
 */
function parseFields(value, defaultValue) {
    if (!value)
        return defaultValue;
    ...
    const fields = value
        .split(',')
        .map((f) => f.trim())
        .filter((f) => f && ENCRYPTABLE_PII_FIELDS.includes(f));
    return fields.length > 0 ? fields : defaultValue;
}
/**
 ...
 */
export function getEncryptionConfigFromEnv(env) {
    return {
        PII_ENCRYPTION_ENABLED: parseBool(env.ENABLE_PII_ENCRYPTION, DEFAULT_ENCRYPTION_CONFIG.PII_ENCRYPTION_ENABLED),
        PII_ENCRYPTION_ALGORITHM: parseAlgorithm(env.PII_ENCRYPTION_ALGORITHM, DEFAULT_ENCRYPTION_CONFIG.PII_ENCRYPTION_ALGORITHM),
        PII_ENCRYPTION_FIELDS: parseFields(env.PII_ENCRYPTION_FIELDS, DEFAULT_ENCRYPTION_CONFIG.PII_ENCRYPTION_FIELDS),
        PII_ENCRYPTION_KEY_VERSION: parseNumber(env.PII_ENCRYPTION_KEY_VERSION, DEFAULT_ENCRYPTION_CONFIG.PII_ENCRYPTION_KEY_VERSION),
    };
}
/**
...
export class EncryptionKeyInvalidError extends Error {
    constructor(reason) {
        super(`PII_ENCRYPTION_KEY format is invalid: ${reason}. ` +
 ...
 */
function detectWeakKeyPattern(key) {
...
export class EncryptionConfigManager {
     ...
     */
    async shouldEncryptField(field) {
        if (!this.config.PII_ENCRYPTION_ENABLED)
            return false;
        if (this.config.PII_ENCRYPTION_ALGORITHM === 'NONE')
            return false;
        return this.config.PII_ENCRYPTION_FIELDS.includes(field);
    }
    /**
 ...
 */
export function createEncryptionConfigManager(env) {
