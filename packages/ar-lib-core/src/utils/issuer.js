 ...
 */
export async function validateTenantExistsAsync(db, kv, tenantId) {
    ...
    try {
        ...
        const row = await adapter.queryOne('SELECT id FROM tenants WHERE id = ? AND is_active = 1', [tenantId]);
        if (!row) {
 ...
 */
export function getPrimaryTenantId(env) {
 ...
 */
export function usesNakedDomainIssuer(env, tenantId) {
 ...
 */
export function buildIssuerUrl(env, tenantSubdomain) {
...
function normalizeHostCandidate(candidate) {
...
export function getRequestHost(request) {
    ...
    return (normalizeHostCandidate(request.headers.get('Host')) ||
...
export function buildRequestIssuerUrl(request, env, tenantId) {
    const explicitHost = request &&
        (normalizeHostCandidate(request.headers.get('Host')) ||
            normalizeHostCandidate(request.headers.get('X-Authrim-Forwarded-Host')) ||
            normalizeHostCandidate(request.headers.get('X-Forwarded-Host')));
    const requestHost = getRequestHost(request);
    const requestHostname = requestHost?.split(':')[0];
    const shouldIgnoreImplicitLocalhost = !explicitHost &&
        !!requestHostname &&
...
export function buildRequestIdentifier(request, env, tenantId, configuredIdentifier) {
 ...
 */
export function isMultiTenantEnabled(env) {
 ...
 */
export function validateHostHeader(host, env) {
