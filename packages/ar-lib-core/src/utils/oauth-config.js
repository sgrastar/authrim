...
export const CONFIG_METADATA = {
    ...
    USERINFO_REQUIRE_OPENID_SCOPE: {
        ...
        description: 'Require openid scope for UserInfo endpoint (OIDC compliance). Set false for OAuth 2.0 compatibility.',
    },
    USER_CACHE_TTL: {
        type: 'number',
    ...
    CONFIG_CACHE_TTL: {
        ...
        description: 'In-memory cache TTL for config values in seconds. Higher = fewer KV reads (cost saving), Lower = faster config propagation. Applied on next Worker instance.',
        min: 10,
        max: 3600,
        unit: 'seconds',
    },
 ...
 *
 * Environment variable mapping (new naming convention):
 * - ACCESS_TOKEN_EXPIRY → TOKEN_EXPIRY (internal)
 * - AUTH_CODE_EXPIRY → AUTH_CODE_TTL (internal)
 * - ENABLE_REFRESH_TOKEN_ROTATION → REFRESH_TOKEN_ROTATION_ENABLED (internal)
 * - ENABLE_STATE_REQUIRED → STATE_REQUIRED (internal)
 * - ENABLE_USERINFO_REQUIRE_OPENID_SCOPE → USERINFO_REQUIRE_OPENID_SCOPE (internal)
 */
export function getConfigFromEnv(env) {
    return {
        TOKEN_EXPIRY: parseNumber(env.ACCESS_TOKEN_EXPIRY, DEFAULT_CONFIG.TOKEN_EXPIRY),
        AUTH_CODE_TTL: parseNumber(env.AUTH_CODE_EXPIRY, DEFAULT_CONFIG.AUTH_CODE_TTL),
        ...
        REFRESH_TOKEN_ROTATION_ENABLED: parseBool(env.ENABLE_REFRESH_TOKEN_ROTATION, DEFAULT_CONFIG.REFRESH_TOKEN_ROTATION_ENABLED),
        ...
        STATE_REQUIRED: parseBool(env.ENABLE_STATE_REQUIRED, DEFAULT_CONFIG.STATE_REQUIRED),
        USERINFO_REQUIRE_OPENID_SCOPE: parseBool(env.ENABLE_USERINFO_REQUIRE_OPENID_SCOPE, DEFAULT_CONFIG.USERINFO_REQUIRE_OPENID_SCOPE),
...
export class OAuthConfigManager {
     ...
     */
    constructor(env, kv = null, cacheTTL = DEFAULT_CONFIG_CACHE_TTL_MS) {
    ...
    getConfigSync() {
        ...
        for (const name of CONFIG_NAMES) {
            const cached = this.cache.get(name);
            config[name] = (cached && cached.expiresAt > Date.now() ? cached.value : this.envConfig[name]);
        }
        return config;
    }
    /**
     ...
     */
    async getConsentGranularScopes() {
        // Try KV first
        if (this.kv) {
            try {
        ...
        const envValue = this.envConfig.CONSENT_GRANULAR_SCOPES;
        return envValue?.toLowerCase() === 'true' || envValue === '1';
    }
    /**
     * Check if consent expiration is enabled
     */
    async getConsentExpirationEnabled() {
        // Try KV first
        if (this.kv) {
            try {
     ...
     */
    async getConsentDefaultExpirationDays() {
        // Try KV first
        if (this.kv) {
            try {
     ...
     */
    async getConsentVersioningEnabled() {
        // Try KV first
        if (this.kv) {
            try {
     ...
     */
    async getConsentDataExportEnabled() {
        // Try KV first
        if (this.kv) {
            try {
     ...
     */
    async getConsentDataExportSyncThresholdKB() {
        // Try KV first
        if (this.kv) {
            try {
 ...
 */
export function createOAuthConfigManager(env, cacheTTL) {
 ...
 */
export async function createOAuthConfigManagerAsync(env) {
    const kv = env.AUTHRIM_CONFIG ?? null;
 ...
 */
export function getOAuthConfigManager(env) {
