 ...
 */
export function getTenantIdFromHost(host, env) {
 ...
 */
export function resolveTenantFromRequest(request, env) {
 ...
 */
export function getTenantIdOrThrow(host, env) {
 ...
 */
export function buildDOKey(resourceType, resourceId, tenantId = DEFAULT_TENANT_ID) {
 ...
 */
export function buildKVKey(prefix, key, tenantId = DEFAULT_TENANT_ID) {
 ...
 */
export function buildDOInstanceName(resourceType, tenantId = DEFAULT_TENANT_ID) {
 ...
 * @returns Tenant-prefixed instance name
 */
export function buildDOInstanceNameForTenant(tenantId, resourceType) {
    return `tenant:${tenantId}:${resourceType}`;
}
/**
 ...
 * @returns Tenant-prefixed key string
 */
export function buildKVKeyForTenant(tenantId, prefix, key) {
    return `tenant:${tenantId}:${prefix}:${key}`;
}
/**
 ...
 */
export function getAuthCodeShardIndex(userId, clientId, shardCount = DEFAULT_CODE_SHARD_COUNT) {
 ...
 */
export function createShardedAuthCode(shardIndex, randomCode) {
 ...
 */
export function parseShardedAuthCode(code) {
 ...
 */
export function buildAuthCodeShardInstanceName(shardIndex, tenantId = DEFAULT_TENANT_ID) {
 ...
 */
export function remapShardIndex(parsedShardIndex, currentShardCount) {
 ...
 */
async function getCurrentShardCount(env) {
 ...
 * @returns Current shard count
 */
export async function getShardCount(env) {
    const now = Date.now();
 ...
 */
let cachedSessionShardCount = null;
 ...
 */
async function getCurrentSessionShardCount(env) {
    // KV takes priority if available (dynamically changeable)
    if (env.AUTHRIM_CONFIG) {
 ...
 * @returns Current session shard count
 */
export async function getSessionShardCount(env) {
    const now = Date.now();
    // Return cached value if within TTL
 ...
 * @returns DO instance name for the shard
 */
export function buildSessionShardInstanceName(shardIndex, tenantId = DEFAULT_TENANT_ID) {
...
export { buildIssuerUrl, isMultiTenantEnabled, validateHostHeader, extractSubdomain, } from './issuer';
