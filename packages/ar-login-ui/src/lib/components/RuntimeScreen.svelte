<script lang="ts">
	...
	type RuntimeField = {
		...
		display_condition?: RuntimeDisplayCondition | null;
		layout_columns?: number | null;
		layout_column?: number | null;
	};

	type RuntimeDisplayCondition = {
		mode: 'always' | 'feature_enabled' | 'hidden';
		feature?: AuthMethod | null;
	};
	...
	type AuthMethod =
		...
		| 'external_idp'
		| 'directory_password';
	type AuthAction = 'send_mail_otp' | 'resend_mail_otp' | 'start_totp' | 'verify_code' | 'back';
	...
	function normalizeField(value: unknown): RuntimeField | null {
		...
		return {
			...
			display_condition: normalizeDisplayCondition(value.display_condition),
			layout_columns: readLayoutInteger(value.layout_columns),
			layout_column: readOptionalLayoutInteger(value.layout_column)
		};
	}

	function normalizeDisplayCondition(value: unknown): RuntimeDisplayCondition | null {
		if (!isRecord(value)) return null;
		if (value.mode === 'hidden') return { mode: 'hidden' };
		if (value.mode === 'feature_enabled') {
			const feature = readString(value.feature);
			if (
				feature === 'passkey' ||
				feature === 'mail_otp' ||
				feature === 'mail_otp_totp' ||
				feature === 'totp' ||
				feature === 'external_idp' ||
				feature === 'directory_password'
			) {
				return { mode: 'feature_enabled', feature };
			}
			return { mode: 'feature_enabled', feature: 'passkey' };
		}
		return null;
	}

	function normalizeScreen(value: Record<string, unknown> | null): RuntimeScreen | null {
	...
	function isEmailIdentityField(field: RuntimeField): boolean {
	...
	function shouldRenderField(field: RuntimeField): boolean {
		...
		if (condition.mode === 'feature_enabled') {
			return condition.feature ? authMethodAvailable(condition.feature) : false;
		}
		return true;
	}
	...
	function shouldRenderLayoutField(field: RuntimeField): boolean {
		...
		if (isSecurityVerificationField(field)) {
			return shouldShowSecurityVerification(field) && Boolean(humanVerificationSiteKey);
		}
		return true;
	}
	...
	function fieldStringValue(field: RuntimeField): string {
		...
		if (isEmailIdentityField(field) && typeof fieldValues.email === 'string') {
			return fieldValues.email;
		}
		return '';
	}

	function fieldError(field: RuntimeField): string {
		const error = fieldErrors[field.field];
		if (error) return error;
		return isEmailIdentityField(field) ? (fieldErrors.email ?? '') : '';
	}
	...
	function codeInputLength(method: 'mail_otp' | 'totp'): number {
		const configured = numberField(
			method === 'mail_otp' ? 'mail_otp_code_length' : 'totp_code_length',
			6
		);
	...
	function buildLayoutSections(fields: RuntimeField[]): RuntimeLayoutSection[] {
		...
		for (const [index, field] of fields.entries()) {
			if (field.block_type === 'layout_row') {
				currentVisible = shouldRenderField(field);
				current = {
			...
			if (!currentVisible || !shouldRenderLayoutField(field)) continue;
...
{#snippet runtimeField(field: RuntimeField)}
	...
	{:else if blockType === 'text'}
		<p class="runtime-screen-text">{field.text || field.label}</p>
	{:else if blockType === 'consent_widget'}
		<div class="runtime-screen-consent-widget">
			<div>
	...
	{:else if blockType === 'divider'}
		...
		<div class="runtime-screen-divider" class:has-label={Boolean(label)}>
			{#if label}
				<span>{label}</span>
			{/if}
		</div>
	...
	{:else if blockType === 'code_input_widget'}
		...
		<div class="runtime-code-input-widget">
			{#if field.text}
				<p>{field.text}</p>
			{/if}
			<div class="runtime-screen-field">
				...
				<PinCodeInput
					...
					label={field.label || $LL.login_totpCodeLabel()}
					digitLabel={(position) => $LL.emailCode_digitLabel({ position })}
					onValueChange={(nextValue) => onFieldValueChange?.(codeInputFieldName(method), nextValue)}
				/>
			</div>
			{#if method === 'mail_otp'}
			...
			{#if fieldErrors[method === 'mail_otp' ? 'mail_otp_code' : 'totp_code']}
				<small class="runtime-screen-error"
	...
	{:else}
		<label class="runtime-screen-field">
			...
			{:else}
				<input
					{disabled}
					type={isEmailIdentityField(field) ? 'email' : 'text'}
					name={isEmailIdentityField(field) ? 'email' : undefined}
					autocomplete={isEmailIdentityField(field) ? 'email' : undefined}
			...
			{/if}
			{#if fieldError(field)}
				<small class="runtime-screen-error">{fieldError(field)}</small>
			{/if}
		</label>
	{/if}
{/snippet}
...
{#if normalizedScreen}
	<div class="runtime-screen" class:is-wide={normalizedScreen.settings?.canvas_layout === 'wide'}>
...
<style>
	...
	.runtime-layout-section {
		display: grid;
		gap: var(--auth-layout-section-gap, 1rem);
		align-items: start;
		width: 100%;
	}

	.runtime-layout-section.has-columns {
		column-gap: var(--auth-layout-column-gap, 0.875rem);
		row-gap: var(--auth-layout-section-gap, 1rem);
	}
	...
	.runtime-screen-field small.runtime-screen-error {
	...
	.runtime-screen-heading h2 {
		...
		font-size: var(--auth-heading-size, 1.375rem);
		line-height: 1.25;
	}

	.runtime-screen-field {
		display: grid;
		gap: var(--auth-field-gap, 0.5rem);
		color: var(--text-primary);
		font-size: var(--auth-label-size, 1rem);
		font-weight: 700;
	}
	...
	.runtime-screen-field input {
		...
		font: inherit;
		font-weight: 500;
		padding: var(--auth-control-padding-y, 0.875rem) var(--auth-control-padding-x, 1rem);
		font-size: var(--auth-control-font-size, 1rem);
	}
	...
	.runtime-code-input-widget > .runtime-screen-error {
		color: var(--danger, #ef4444);
		font-size: var(--auth-copy-size, 0.875rem);
	}
	...
	.runtime-code-actions {
		...
		gap: var(--auth-widget-gap, 0.625rem);
	}
	...
	.runtime-auth-button {
		...
		width: 100%;
		min-width: 0;
		min-height: var(--auth-control-height, 3rem);
		border: 0;
		border-radius: var(--button-radius, var(--radius-full, 999px));
		background: var(--button-primary-bg, var(--gradient-primary));
		color: var(--button-primary-text, white);
		font: inherit;
		font-size: var(--auth-control-font-size, 1rem);
		font-weight: var(--auth-control-font-weight, 700);
		padding: var(--auth-control-padding-y, 0.875rem) var(--auth-control-padding-x, 1.25rem);
		line-height: 1.35;
		text-align: center;
		white-space: nowrap;
	}
	...
	.runtime-auth-provider-icon {
		width: var(--auth-provider-icon-size, 1.25rem);
		height: var(--auth-provider-icon-size, 1.25rem);
		object-fit: contain;
		flex: 0 0 var(--auth-provider-icon-size, 1.25rem);
	}
	...
	.runtime-screen-consent-widget > div {
		display: inline-flex;
		align-items: center;
		gap: 0.5rem;
	}

	.runtime-screen-consent-widget p {
		margin: 0;
		color: var(--text-secondary);
		font-size: var(--auth-copy-size, 0.875rem);
		line-height: 1.5;
	...
	.runtime-consent-content {
		color: var(--text-secondary);
		font-size: var(--auth-copy-size, 0.9rem);
		line-height: 1.5;
		min-width: 0;
	}
