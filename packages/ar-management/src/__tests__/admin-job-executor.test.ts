...
import { buildTenantDatabaseBindingPlan, encryptObjectArtifact } from '@authrim/ar-lib-core';
...
describe('generic admin job executor', () => {
  ...
  it('executes tenant database provisioning through Cloudflare API when requested', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, result: { bindings: [] } }))
      )
      .mockResolvedValueOnce(
        new Response(
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, result: { uuid: 'core-db-id' } }))
      )
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ success: true, result: [], result_info: { page: 1, total_pages: 1 } })
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, result: { uuid: 'pii-db-id' } }))
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, result: { bindings: [] } }))
      )
    ...
    await processPendingGenericAdminJobs(
      {
        ...
        TENANT_D1_DEPLOYMENT_WORKER_SCRIPTS: 'ar-auth',
      } as never,
      logger
    );
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.cloudflare.com/client/v4/accounts/account-id/workers/scripts/ar-auth/settings',
      expect.objectContaining({ method: 'PATCH' })
    );
    const patchCall = fetchMock.mock.calls.find(
      ([url, init]) =>
        String(url).includes('/workers/scripts/ar-auth/settings') &&
        (init as RequestInit).method === 'PATCH'
    );
    const patchBody = JSON.parse(String((patchCall?.[1] as RequestInit).body)) as {
      bindings: Array<{ type: string; name: string; database_id: string }>;
    };
    expect(patchBody.bindings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'd1', database_id: 'core-db-id' }),
        expect.objectContaining({ type: 'd1', database_id: 'pii-db-id' }),
      ])
    );
    const finalUpdate = mockAdapter.execute.mock.calls.at(-1);
    expect(finalUpdate?.[0]).toEqual(expect.stringContaining("SET status = 'processing'"));
    const progressJson = (finalUpdate?.[1] as unknown[])[0] as string;
    expect(JSON.parse(progressJson)).toMatchObject({
      stage: 'cloudflare_api_deployed_pending_migrations_registry_and_snapshots',
      cloudflare_api: {
        d1_databases_created: 2,
        worker_scripts_patched: [{ script_name: 'ar-auth', patched_binding_count: 2 }],
      },
      next_required_steps: [
  ...
  it('fails Cloudflare API provisioning before patching when a generated binding name already exists with another type', async () => {
    const conflictingBinding = buildTenantDatabaseBindingPlan({
      environment: 'dev',
      tenantId: 'tenant-a',
      tenantSlug: 'tenant-a',
      role: 'tenant_core',
    ...
    const fetchMock = vi.fn().mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          success: true,
          result: {
            bindings: [{ type: 'kv_namespace', name: conflictingBinding, namespace_id: 'kv-1' }],
          },
        })
      )
    );
    ...
    expect((finalUpdate?.[1] as unknown[])[0]).toContain('tenant_d1_worker_binding_conflict');
    expect((finalUpdate?.[1] as unknown[])[0]).toContain(conflictingBinding);
  });
