...
describe('admin settings metadata APIs', () => {
  ...
  describe('tenant clone', () => {
    const validBody = { id: 'destination', name: 'Destination' };

    it('returns the platform authority rejection before reading tenant data', async () => {
      mocks.authority.mockResolvedValueOnce(Response.json({ error: 'forbidden' }, { status: 403 }));
      expect((await adminTenantCloneHandler(context({ body: validBody }))).status).toBe(403);
      expect(mocks.tenantGuard).not.toHaveBeenCalled();
    });
    ...
    it('blocks cloning in single-tenant mode', async () => {
      mocks.singleTenant.mockReturnValueOnce(true);
      expect((await adminTenantCloneHandler(context({ body: validBody }))).status).toBe(400);
      expect(mocks.singleTenantError).toHaveBeenCalled();
    });
    ...
    it.each([
      ...
      [{ id: '-bad', name: 'Name' }],
      [{ id: 'destination', name: '' }],
      [{ id: 'destination', name: 'Name', include_clients: 'yes' }],
    ])('rejects an invalid clone request %#', async (body) => {
      expect((await adminTenantCloneHandler(context({ body }))).status).toBe(400);
    });

    it('does not clone a missing source tenant', async () => {
      expect((await adminTenantCloneHandler(context({ body: validBody }))).status).toBe(404);
    });

    it('rejects a destination tenant ID already in use', async () => {
      mocks.adapter.queryOne
        .mockResolvedValueOnce({ id: 'source', name: 'Source' })
        .mockResolvedValueOnce({ id: 'destination' });
      expect((await adminTenantCloneHandler(context({ body: validBody }))).status).toBe(400);
      expect(mocks.adapter.execute).not.toHaveBeenCalled();
    });

    it('creates a tenant and clones roles by default', async () => {
      mocks.adapter.queryOne
        .mockResolvedValueOnce({ id: 'source', name: 'Source' })
      ...
      mocks.adapter.query.mockResolvedValueOnce([
        ...
        },
        {
          ...
          hierarchy_level: 1,
          is_assignable: 1,
          parent_role_id: 'missing',
          display_name: null,
          is_system: 0,
        },
      ]);
      const response = await adminTenantCloneHandler(
        context({ body: { ...validBody, description: 'copy' } })
      );
      const body = (await response.json()) as { cloned_items: Record<string, number | boolean> };
      expect(response.status).toBe(201);
      expect(body.cloned_items).toEqual({ settings: true, clients: 0, roles: 3, webhooks: 0 });
      expect(mocks.adapter.execute).toHaveBeenCalledTimes(4);
      expect(mocks.audit).toHaveBeenCalledWith(
        expect.anything(),
        'tenant.cloned',
        'tenant',
        'destination',
        expect.objectContaining({ source_tenant_id: 'source' })
      );
    });

    it('optionally clones clients and webhooks while skipping roles', async () => {
      mocks.adapter.queryOne
        .mockResolvedValueOnce({ id: 'source', name: 'Source' })
        .mockResolvedValueOnce(null);
      mocks.adapter.query
        .mockResolvedValueOnce([{ client_id: 'client-1', client_name: 'Client' }])
        .mockResolvedValueOnce([
          {
            name: 'Events',
      ...
      await expect(response.json()).resolves.toMatchObject({
        cloned_items: { clients: 1, roles: 0, webhooks: 1 },
      });
      expect(mocks.adapter.execute).toHaveBeenCalledTimes(3);
    });

    it('returns internal_error and audits nothing when persistence fails', async () => {
      mocks.adapter.queryOne
        .mockResolvedValueOnce({ id: 'source', name: 'Source' })
        .mockResolvedValueOnce(null);
      mocks.adapter.execute.mockRejectedValueOnce(new Error('D1 unavailable'));
      expect((await adminTenantCloneHandler(context({ body: validBody }))).status).toBe(500);
