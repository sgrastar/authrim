...
describe('logout webhook configuration', () => {
  ...
  it.each([null, '{', JSON.stringify({ enabled: true, retry: { max_attempts: 7 } })])(
    ...
    async (stored) => {
      ...
      const body = (await response.json()) as {
        config: typeof DEFAULT_LOGOUT_WEBHOOK_CONFIG;
        source: string;
      };
      expect(body.source).toBe(stored && stored !== '{' ? 'kv' : 'default');
      expect(body.config.retry.initial_delay_ms).toBe(
        DEFAULT_LOGOUT_WEBHOOK_CONFIG.retry.initial_delay_ms
      );
  ...
  it('gets defaults without KV and sanitizes read errors', async () => {
    await expect((await getLogoutWebhookConfig(context())).json()).resolves.toMatchObject({
      source: 'default',
    });
    const store = kv();
    store.get.mockRejectedValueOnce(new Error('secret KV detail'));
  ...
  it('requires KV and valid JSON for update', async () => {
    ...
    expect(
      (await updateLogoutWebhookConfig(context({ store: kv(), bodyError: true }))).status
    ).toBe(400);
  });
  ...
  it.each([
    ...
    [{ retry: { initial_delay_ms: 99 } }],
    [{ retry: { initial_delay_ms: 60001 } }],
    [{ retry: { max_delay_ms: '1000' } }],
    [{ retry: { max_delay_ms: 999 } }],
    [{ retry: { max_delay_ms: 300001 } }],
    [{ retry: { backoff_multiplier: '2' } }],
    [{ retry: { backoff_multiplier: 0.9 } }],
    [{ retry: { backoff_multiplier: 5.1 } }],
  ...
  it('deep-merges a valid partial retry configuration', async () => {
    const store = kv(
      JSON.stringify({ enabled: false, retry: { max_attempts: 2, initial_delay_ms: 500 } })
    );
    const body = {
      enabled: true,
      request_timeout_ms: 1000,
  ...
  it.each([null, '{'])('uses defaults when existing config is %s', async (stored) => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      config: { enabled: true, retry: DEFAULT_LOGOUT_WEBHOOK_CONFIG.retry },
    });
  });

  it('preserves existing retry when omitted and sanitizes write errors', async () => {
    const store = kv(
      JSON.stringify({ retry: { ...DEFAULT_LOGOUT_WEBHOOK_CONFIG.retry, max_attempts: 9 } })
    );
    const response = await updateLogoutWebhookConfig(context({ store, body: { enabled: true } }));
    await expect(response.json()).resolves.toMatchObject({
  ...
  it('resets to defaults, requires KV, and sanitizes delete errors', async () => {
    ...
    const store = kv();
    await expect(
      (await resetLogoutWebhookConfig(context({ store }))).json()
