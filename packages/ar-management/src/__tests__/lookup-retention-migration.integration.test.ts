...
describe('Lookup retention and OTP bucket migration', () => {
  ...
  beforeEach(() => {
    ...
    database.exec(migration('001_lookup_directory.sql'));
    database.exec(migration('002_identifier_replacement_verification_gate.sql'));
    database.exec(migration('003_allow_external_subject_identifier_replacement.sql'));
  });

  afterEach(() => database.close());

  it('backfills legacy aliases and challenge buckets and enforces future bucket writes', () => {
    database.exec(
      `INSERT INTO lookup_tenant_aliases (
         ...
         'disabled', 'disabled', 'disabled', 100, 150
       );
       INSERT INTO lookup_discovery_otp_challenges (
         ...
         '${'b'.repeat(64)}', 1, '${'c'.repeat(64)}', 'sent', 0, 5, 1000, NULL, 100, 100
       );`
    );

    database.exec(migration('004_lookup_retention_and_otp_bucket.sql'));
  ...
  it('fails closed when a retained challenge cannot be assigned to an exact bucket', () => {
    database.exec(
      `INSERT INTO lookup_discovery_otp_challenges (
         challenge_id, normalization_version, email_blind_digest, hmac_key_generation,
         otp_verifier, delivery_state, attempt_count, attempt_limit, expires_at,
    ...
    expect(() => database.exec(migration('004_lookup_retention_and_otp_bucket.sql'))).toThrow(
