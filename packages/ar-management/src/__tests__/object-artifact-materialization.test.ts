...
import {
  materializeEncryptedObjectArtifact,
} from '../object-artifact-materialization';
...
describe('object artifact materialization', () => {
  it('creates manifest and chunk objects when payload exceeds the chunk budget', async () => {
    ...
    const result = await materializeEncryptedObjectArtifact(
      { execute } as any,
      bucket,
      {
        tenantId: 'default',
    ...
    expect((bucket.put as any).mock.calls.some((call: unknown[]) => String(call[0]).includes('.part-000000'))).toBe(true);
