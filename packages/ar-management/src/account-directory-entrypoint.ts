...
const EXPOSED_ERROR =
  /^(backup_mutation_unavailable|account_directory_(rpc_caller_unauthorized|source_route_invalid|source_binding_unavailable)|directory_[a-z0-9_]+|invalid_[a-z0-9_]+|lookup_registry_[a-z0-9_]+|control_plane_sensitive_[a-z0-9_:.$-]+)$/u;
        ...
        async () => {
          ...
          const coordinator = new AccountDirectoryCoordinator({
            ...
            onAccountActivated: async (activatedPublication, now) => {
              ...
              await processAccountLifecycleEventForOperation(this.env, tenantCore, {
                tenantId: activatedPublication.tenantId,
          ...
          return coordinator.publish(publication);
        }
      );
    } catch (error) {
    ...
    try {
      ...
      return await runTenantBackupCoveredEffect(
        ...
        async () => {
          ...
          return coordinator.remove(publication);
        }
      );
    } catch (error) {
