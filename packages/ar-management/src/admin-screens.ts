...
function normalizeLocalizations(value: unknown): Record<string, ScreenLocalization> {
  ...
  return value as Record<string, ScreenLocalization>;
}
...
function toResponse(row: Row): ScreenResponse {
  return {
    ...
    fields: parseJson<ScreenField[]>(row.fields_json, []),
    localizations: parseJson<Record<string, ScreenLocalization>>(row.localizations_json, {}),
...
async function ensureDefaultScreens(c: AdminContext, tenantId: string): Promise<void> {
  ...
  for (const screen of DEFAULT_SCREENS) {
    ...
    if (existing) {
      if (readBoolean(existing.is_system, false)) {
        ...
        const currentLocalizations = parseJson<Record<string, ScreenLocalization>>(
...
export async function adminScreenUpdateHandler(c: AdminContext) {
  ...
  try {
    ...
    const authCtx = createAuthContextFromHono(c, tenantId);
    const existing = await authCtx.coreAdapter.queryOne<Row>(
      'SELECT id, is_system FROM screens WHERE tenant_id = ? AND id = ?',
      [tenantId, id]
    );
