<script lang="ts">
	...
	const label = $derived(data?.label || 'MFA');
	const factor = $derived(data?.config?.factor || 'totp');
...
<div class="mfa-node" class:selected>
	...
	<div class="node-body">
		<span class="config-value">{factor}</span>
	</div>
