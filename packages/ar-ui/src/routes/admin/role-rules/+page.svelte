...
<div class="admin-page">
	<div class="page-header">
		<div class="page-header-info">
			<h1 class="page-title">Role Assignment Rules</h1>
				...
				<tbody>
					{#each rules as rule (rule.id)}
						<tr>
							...
							<td>
								<span class="badge-active {rule.is_active ? 'active' : 'inactive'}">
									{rule.is_active ? 'Active' : 'Inactive'}
								</span>
							</td>
							<td>
								<div class="table-actions">
									<button class="btn-test" onclick={(e) => openTestDialog(rule, e)}>Test</button>
									<button class="btn btn-ghost btn-sm" onclick={(e) => toggleActive(rule, e)}>
		...
		>
			...
			<div class="modal-body">
				...
				<div class="form-group">
					...
					<textarea id="test-claims" class="form-textarea" bind:value={testClaims} rows="6"
					></textarea>
					<span class="form-hint">Enter the claims object that would be received from the IdP</span>
				</div>

				{#if testResult}
					<div class="test-result-box {testResult.matched ? 'matched' : 'not-matched'}">
