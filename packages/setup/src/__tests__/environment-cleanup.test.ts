...
describe('cleanupLocalEnvironmentArtifacts', () => {
  ...
  it('removes local env directories and package wrangler sections for the deleted env', async () => {
    ...
    const adminUiDir = join(packagesDir, 'ar-admin-ui');
    const loginUiDir = join(packagesDir, 'ar-login-ui');

    await mkdir(envDir, { recursive: true });
    await mkdir(externalKeysDir, { recursive: true });
    await mkdir(samlDir, { recursive: true });
    await mkdir(adminUiDir, { recursive: true });
    await mkdir(loginUiDir, { recursive: true });
...
`,
    ...
    await writeFile(
      join(loginUiDir, 'wrangler.toml'),
      `# Auto-generated by @authrim/setup - do not edit manually
name = "single-ar-login-ui"
workers_dev = true
`,
      'utf-8'
    );
    ...
    expect(existsSync(join(adminUiDir, 'wrangler.toml'))).toBe(false);
    expect(existsSync(join(loginUiDir, 'wrangler.toml'))).toBe(true);
    expect(progress.at(-1)).toContain('Removed environment directory');
  });

  it('preserves hand-authored and other-environment UI wrangler files', async () => {
    const baseDir = await mkdtemp(join(tmpdir(), 'authrim-cleanup-ui-ownership-'));
    tempDirs.push(baseDir);

    const envDir = join(baseDir, '.authrim', 'test');
    const adminUiDir = join(baseDir, 'packages', 'ar-admin-ui');
    const loginUiDir = join(baseDir, 'packages', 'ar-login-ui');
    await mkdir(envDir, { recursive: true });
    await mkdir(adminUiDir, { recursive: true });
    await mkdir(loginUiDir, { recursive: true });
    await writeFile(join(envDir, 'config.json'), '{}', 'utf-8');
...
`,
    ...
    expect(result.errors).toEqual([]);
    await expect(readFile(join(adminUiDir, 'wrangler.toml'), 'utf-8')).resolves.toContain(
      'Maintained by the operator'
    );
    await expect(readFile(join(loginUiDir, 'wrangler.toml'), 'utf-8')).resolves.toContain(
      'production-ar-login-ui'
    );
  });
