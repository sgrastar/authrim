...
const UI_WORKER_COMPONENTS = ['ar-admin-ui', 'ar-login-ui'] as const;
const GENERATED_UI_WRANGLER_HEADER = '# Auto-generated by @authrim/setup - do not edit manually';

function getGeneratedUiWranglerWorkerName(content: string): string | null {
  const lines = content.split(/\r?\n/);
  if (lines[0]?.trim() !== GENERATED_UI_WRANGLER_HEADER) {
    return null;
  }
...
export async function cleanupLocalEnvironmentArtifacts(
  ...
  if (packagesDir && existsSync(packagesDir)) {
    ...
    // UI Workers use one generated top-level wrangler.toml per package instead of an
    // environment section. Remove it only when both the setup marker and exact Worker name prove
    // that the file belongs to the deleted environment. A config generated for another
    // environment, or a hand-authored file, must remain untouched.
    for (const component of UI_WORKER_COMPONENTS) {
      ...
      try {
        const currentContent = await readFile(deployPath, 'utf-8');
        if (getGeneratedUiWranglerWorkerName(currentContent) !== `${env}-${component}`) {
          continue;
        }

        await rm(deployPath, { force: true });
        if (existsSync(deployPath)) {
          throw new Error('ui_wrangler_cleanup_reflection_failed');
        }
        removed.push(deployPath);
        onProgress?.(`  🧹 Removed generated ${component}/wrangler.toml`);
      } catch (error) {
        const message = `Failed to clean wrangler.toml for ${component}: ${error instanceof Error ? error.message : String(error)}`;
        errors.push(message);
        onProgress?.(`  ⚠️ ${message}`);
