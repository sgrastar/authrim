...
describe('RefreshTokenRotator', () => {
  ...
  describe('Token Family Creation', () => {
    ...
    it('should initialize family with rotation count 0', async () => {
      ...
      const infoBody = await infoResponse.json() as { rotationCount: number; tokenCount: { current: number; previous: number } };
  ...
  describe('Atomic Token Rotation', () => {
    it('should rotate token successfully', async () => {
      ...
      const body = await response.json() as { newToken: string; rotationCount: number; familyId: string; expiresIn: number };
    ...
    it('should increment rotation count on each rotation', async () => {
      ...
      const infoResponse = await rotator.fetch(infoRequest);
      const infoBody = await infoResponse.json() as { rotationCount: number; tokenCount: { previous: number } };
  ...
  describe('Theft Detection (Replay Attack)', () => {
    it('should detect token replay and revoke family', async () => {
      ...
      const replayBody = await replayResponse.json() as { error: string; error_description: string; action: string };
  ...
  describe('Family Info Endpoint', () => {
    it('should return family information without exposing tokens', async () => {
      ...
      const body = await response.json() as { id: string; userId: string; clientId: string; scope: string; rotationCount: number; createdAt: number; lastRotation: number; expiresAt: number; tokenCount: unknown; currentToken?: unknown; previousTokens?: unknown };
  ...
  describe('Health Check and Status', () => {
    it('should return status endpoint with stats', async () => {
      ...
      const body = await response.json() as { status: string; families: unknown; tokens: unknown; config: { defaultTtl: number; maxPreviousTokens: number }; timestamp: number };
    ...
    it('should track active vs expired families', async () => {
      ...
      const body = await response.json() as { families: { total: number; active: number; expired: number } };
