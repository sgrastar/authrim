 ...
 */
export async function scimAuthMiddleware(c, next) {
 ...
 */
async function validateScimToken(env, token) {
    try {
 ...
 */
function scimErrorResponse(c, status, detail, scimType) {
    const error = {
 ...
 */
export async function generateScimToken(env, options = {}) {
 ...
 * Revoke a SCIM token
 */
export async function revokeScimToken(env, tokenHash) {
    try {
 ...
 */
export async function listScimTokens(env) {
    const tokens = [];
    try {
 ...
 */
export async function validateScimTokenFromDB(db, token) {
    try {
        const tokenHash = await hashToken(token);
        const result = await db
            .prepare(`SELECT id, expires_at, enabled FROM scim_tokens
