 ...
 */
export function userToScim(user, context) {
    ...
    // Build name object
    const name = user.given_name || user.family_name || user.middle_name
        ? {
    ...
    const phoneNumbers = user.phone_number
        ? [
            {
 ...
 */
export function scimToUser(scimUser) {
    const user = {};
    if (scimUser.externalId)
        user.external_id = scimUser.externalId;
    if (scimUser.userName)
        user.preferred_username = scimUser.userName;
    if (scimUser.active !== undefined)
        user.active = scimUser.active ? 1 : 0;
    // Name fields
    if (scimUser.name) {
        if (scimUser.name.givenName)
            user.given_name = scimUser.name.givenName;
        if (scimUser.name.familyName)
            user.family_name = scimUser.name.familyName;
        if (scimUser.name.middleName)
            user.middle_name = scimUser.name.middleName;
    ...
    if (scimUser.nickName)
        user.nickname = scimUser.nickName;
    if (scimUser.profileUrl)
        user.profile = scimUser.profileUrl;
    if (scimUser.preferredLanguage)
        user.locale = scimUser.preferredLanguage;
    if (scimUser.timezone)
        user.zoneinfo = scimUser.timezone;
    // Email (primary)
    ...
    if (enterpriseExt) {
        const customAttrs = {};
        if (enterpriseExt.employeeNumber)
            customAttrs.employeeNumber = enterpriseExt.employeeNumber;
        if (enterpriseExt.costCenter)
            customAttrs.costCenter = enterpriseExt.costCenter;
        if (enterpriseExt.organization)
            customAttrs.organization = enterpriseExt.organization;
        if (enterpriseExt.division)
            customAttrs.division = enterpriseExt.division;
        if (enterpriseExt.department)
            customAttrs.department = enterpriseExt.department;
        if (enterpriseExt.manager?.value)
            customAttrs.manager = enterpriseExt.manager.value;
 ...
 */
export function groupToScim(group, context, members) {
 ...
 */
export function scimToGroup(scimGroup) {
    const group = {};
    if (scimGroup.externalId)
        group.external_id = scimGroup.externalId;
 ...
 */
export function generateEtag(resource) {
 ...
 */
export function applyPatchOperations(resource, operations) {
 ...
 */
export function validateScimUser(user) {
    const errors = [];
    if (!user.userName) {
 ...
 */
export function validateScimGroup(group) {
    const errors = [];
