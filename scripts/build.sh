...
load_config() {
  ...
  API_WORKER_NAME=$(jq -r '.components.api.worker_name // "authrim"' "$CONFIG_FILE")
  ...
  LOGIN_PAGES_PROJECT=$(jq -r '.components.login_page.pages_project_name // "authrim-login"' "$CONFIG_FILE")
  ...
  ADMIN_PAGES_PROJECT=$(jq -r '.components.admin_page.pages_project_name // "authrim-admin"' "$CONFIG_FILE")
...
generate_dev_vars() {
  ...
  cat > .dev.vars <<EOF
# Authrim Development Environment Variables
...
update_wrangler_toml() {
  ...
  if [[ -n "$db_id" && "$db_id" != "null" ]]; then
    ...
    for toml_file in packages/*/wrangler.toml; do
      if [[ -f "$toml_file" ]] && grep -q "database_name.*=.*\"authrim-" "$toml_file"; then
        if [[ "$VERBOSE" == "true" ]]; then
          echo "[VERBOSE] Updating $toml_file with D1: $db_name ($db_id)"
        fi

        # Use sed to update D1 database configuration
        sed -i.bak "s/database_name = \"authrim-[^\"]*\"/database_name = \"$db_name\"/" "$toml_file"
