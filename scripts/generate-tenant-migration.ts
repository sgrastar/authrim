...
const TENANT_SCOPED_UNIQUE_CONSTRAINTS = [
  {
    table: 'users',
    columns: ['email'],
    oldIndex: 'idx_users_email',
    newIndex: 'idx_users_tenant_email',
  },
];
...
function main() {
  ...
  if (tables.length === 0) {
    console.error('❌ No tables found in migrations directory');
    process.exit(1);
  }
  ...
  console.error('');
  console.error(`✅ Generated migration for ${tables.length} tables:`);
