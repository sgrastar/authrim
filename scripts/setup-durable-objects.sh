...
for i in $(seq 1 $MAX_RETRIES); do
    if (cd packages/shared && pnpm run deploy); then
        ...
        echo "   • Durable Objects: 11 (SessionStore, AuthCodeStore, RefreshTokenRotator, KeyManager, ChallengeStore, RateLimiterCounter, PARRequestStore, DPoPJTIStore, TokenRevocationStore, DeviceCodeStore, CIBARequestStore)"
        echo ""
