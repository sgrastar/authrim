...
test.describe('Authentication Flow', () => {
	test.describe('Login Page', () => {
		test('should display login page elements', async ({ page }) => {
			...
			const passkeyButton = page.locator('button').filter({ hasText: /passkey|webauthn|biometric/i });
	...
	test.describe('Consent Page', () => {
		test('should redirect to login when accessing consent without session', async ({ page }) => {
			...
			const url = page.url();
			expect(url.includes('/login') || url.includes('/consent') || url.includes('/error')).toBeTruthy();
		});
	});
	...
	test.describe('Redirect Handling', () => {
		test('should handle OAuth authorization redirect parameters', async ({ page }) => {
			...
			expect(
				currentUrl.includes('/login') || currentUrl.includes('/authorize') || currentUrl.includes('challenge')
			).toBeTruthy();
		});
	});
});
