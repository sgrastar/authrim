...
import {
  runGeneratedAdminApiSmoke,
  type GeneratedAdminApiSmokeResult,
} from '../../packages/setup/src/core/generated-admin-api-smoke.js';
...
function printUsage(): void {
  process.stdout.write(`Authrim generated admin API smoke checker

Usage:
  pnpm exec tsx test/environment-validation/smoke-generated-admin-api.ts --env <env>
  pnpm exec tsx test/environment-validation/smoke-generated-admin-api.ts --config <path/to/.authrim/{env}/config.json>
...
Options:
  ...
  --client-id <id>          Reuse an existing client_id for check-api-keys smoke
...
function printChecklist(result: GeneratedAdminApiSmokeResult): void {
  process.stdout.write(`\nAuthrim generated admin API smoke\n`);
...
async function main(): Promise<void> {
  ...
  const result = await runGeneratedAdminApiSmoke({
...
void main().catch((error) => {
  ...
  process.stderr.write(`generated admin api smoke failed: ${message}\n`);
  printUsage();
  process.exitCode = 1;
});
