...
import {
  runGeneratedServerSurfacesSmoke,
  type GeneratedServerSurfacesSmokeResult,
} from '../../packages/setup/src/core/generated-server-surfaces-smoke.js';
...
function printUsage(): void {
  process.stdout.write(`Authrim generated server-surface live smoke checker

Usage:
  pnpm exec tsx test/environment-validation/smoke-generated-server-surfaces.ts --env <env>
  pnpm exec tsx test/environment-validation/smoke-generated-server-surfaces.ts --config <path/to/.authrim/{env}/config.json>
...
function printChecklist(result: GeneratedServerSurfacesSmokeResult): void {
  process.stdout.write(`\nAuthrim generated server-surface live smoke\n`);
...
async function main(): Promise<void> {
  ...
  const result = await runGeneratedServerSurfacesSmoke({
...
void main().catch((error) => {
  ...
  process.stderr.write(`generated server-surface smoke failed: ${message}\n`);
  printUsage();
  process.exitCode = 1;
});
