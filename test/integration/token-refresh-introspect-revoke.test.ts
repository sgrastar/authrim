...
describe('refresh, introspection, and revocation lifecycle', () => {
  ...
  async function storeAuthorizationCode(): Promise<string> {
    ...
    // base64url value can occasionally start with `<digits>_`, which the token endpoint correctly
    // interprets as a sharded code and therefore looks up in a different Durable Object instance.
