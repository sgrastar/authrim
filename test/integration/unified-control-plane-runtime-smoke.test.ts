...
describe('unified Control Plane runtime smoke', () => {
  it('routes request-context auth and PII adapters to assigned D1 bindings', async () => {
    ...
    const generationPublishedAt = new Date().toISOString();
    const kv = {
      get: vi.fn(async (key: string) => {
        ...
        if (key === generationKey) {
          return JSON.stringify({
            ...
            routeStatus: 'active',
            quarantineDenyGeneration: 0,
            publishedAt: generationPublishedAt,
            expiresAt: new Date(Date.now() + 30 * 60 * 1000).toISOString(),
          });
        }
    ...
    const fixedPlatformDb = createTenantExistsDb();
    const env = {
    ...
    app.get('/userinfo', async (c) => {
      ...
      c.set('accountDataContext', {
        ...
        userCacheScope: {
          routeGeneration: 7,
          bindingGeneration: 'core:7:pii:7',
          schemaGeneration: 'core:87:pii:12',
        },
    ...
    expect(fixedPlatformDb.prepare).not.toHaveBeenCalled();
