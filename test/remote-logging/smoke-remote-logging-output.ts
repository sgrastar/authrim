...
Usage:
  ...
  pnpm exec tsx test/remote-logging/smoke-remote-logging-output.ts --config .authrim/test/config.json
...
Options:
  ...
  --skip-r2                 Skip Cloudflare R2 object-list checks
...
function parseR2ListPayload(payload: unknown): R2ObjectListItem[] {
  const rows = Array.isArray(payload)
    ? payload
    : isRecord(payload) && Array.isArray(payload.objects)
      ? payload.objects
  ...
  return rows.flatMap((row) => {
    ...
    return [
      {
        key,
        size: numberValue(row.size) ?? undefined,
        uploaded: stringValue(row.uploaded) ?? stringValue(row.created) ?? undefined,
      },
    ];
  });
}

function parseR2ListText(stdout: string): R2ObjectListItem[] {
  return stdout
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line.length > 0 && !line.startsWith('Listing objects'))
    .flatMap((line) => {
      const match = line.match(
        /(?:^|\s)([A-Za-z0-9._~!$&'()*+,;=:@/%-]+\.jsonl(?:\.gz)?)(?:\s|$)/u
      );
      return match?.[1] ? [{ key: match[1] }] : [];
    });
}

function isWranglerR2ObjectListUnsupported(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return (
    message.includes('Unknown arguments: prefix, json, list') ||
    message.includes('wrangler_r2_object_list_unsupported')
  );
}
...
async function listR2Objects(input: {
  ...
  prefix: string;
  timeoutMs: number;
}): Promise<R2ObjectListItem[]> {
  const result = await execa(
    'wrangler',
    ['r2', 'object', 'list', input.bucketName, '--prefix', input.prefix, '--json'],
    {
      all: true,
      reject: false,
      timeout: Math.max(input.timeoutMs, 30_000),
      env: { ...process.env, WRANGLER_SEND_METRICS: 'false' },
    }
  );

  if (result.exitCode !== 0) {
    const output = result.all || result.stderr || result.stdout || 'wrangler_r2_list_failed';
    if (output.includes('Unknown arguments: prefix, json, list')) {
      throw new Error('wrangler_r2_object_list_unsupported');
    }
    throw new Error(output);
  }

  try {
    return parseR2ListPayload(JSON.parse(result.stdout));
  } catch {
    return parseR2ListText(result.stdout);
  }
}
...
async function pollR2Object(input: {
  bucketName: string;
  prefix: string;
  matches: (item: R2ObjectListItem) => boolean;
  timeoutMs: number;
}): Promise<R2ObjectListItem | null> {
  const deadline = Date.now() + Math.max(input.timeoutMs, 45_000);
  let lastObjects: R2ObjectListItem[] = [];
  while (Date.now() < deadline) {
    lastObjects = await listR2Objects({
      bucketName: input.bucketName,
      prefix: input.prefix,
      timeoutMs: input.timeoutMs,
    });
    const match = lastObjects.find(input.matches);
    ...
    if (smokeClient) {
      ...
      if (!auditArchiveBucket) {
        ...
        try {
          ...
          if (object) addPass(archiveCheck, `admin audit archive object observed: ${object.key}`);
          else addFail(archiveCheck, 'no new admin_audit archive object observed');
        } catch (error) {
          if (isWranglerR2ObjectListUnsupported(error)) {
            addWarn(archiveCheck, 'wrangler does not support R2 object listing in this install');
          } else {
      ...
      if (!sensitiveBucket) {
        ...
        try {
          ...
          if (object) addPass(detailCheck, `sensitive-detail object observed: ${object.key}`);
          else addWarn(detailCheck, 'no new admin_audit sensitive-detail object observed');
        } catch (error) {
          if (isWranglerR2ObjectListUnsupported(error)) {
            addWarn(detailCheck, 'wrangler does not support R2 object listing in this install');
          } else {
      ...
      if (!diagnosticBucket) {
        ...
        try {
          ...
          if (isWranglerR2ObjectListUnsupported(error)) {
            addWarn(diagnosticCheck, 'wrangler does not support R2 object listing in this install');
          } else {
